...
| `packages/cli/src/import-resolution.ts` | The one rule for which file a relative or `@/` import lands on: the source ahead of a stale emitted twin, the six extensions, a directory's `package.json` entry and then its index (never the directory itself), and a `.d.ts` only for a type-only import. `guren check --arch` and the route-wiring check both resolve through it; a second resolver is how `'../modules/billing'` came to resolve to a directory no rule matched |
