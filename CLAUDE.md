...
| `packages/cli/src/app-routes.ts` | The one join of the introspected app's routes to the routes file's definitions (RFC 0026 §5, Part 2d), on method, path, name and controller action, nth to nth, and nothing where the two sides count a key differently. `loadIntrospectedRouteDefinitions()` is what `guren context` and `codegen --introspect` read: the manifest decides which routes exist and in which order, and each joined route is the routes file's definition, whose Zod every renderer reads; an unmatched route (a provider's) comes from the manifest without schemas. `route-contract-check` joins through it for its Zod fallback. Codegen stays on the routes file by default, and so does `planAgentManifest()`, the rule for what codegen writes |
