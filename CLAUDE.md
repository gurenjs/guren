...
bunx guren check --fix          # Run each distinct `fix` once (generated files only), then check again and report that run with `fixes`; exits non-zero when a fix command fails
