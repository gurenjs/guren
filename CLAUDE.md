...
| `packages/cli/src/introspect.ts` | The CLI's reader of RFC 0026's manifest: `introspectApp(cwd, { timeoutMs })` spawns `introspect-child.ts` under `GUREN_INTROSPECT=1` with the app root as cwd (Bun loads `.env` as for `guren dev`), memoised per root and timeout per process (`fresh: true` runs a child of its own, which the gate asks for since the dev MCP server calls it for a whole session; `introspectRunner()` turns a `runCheck()`/`runAudit()` `introspect` option into one run shared by its callers; `ADVISORY_INTROSPECT_TIMEOUT_MS`, 10 s, caps the runs that only feed advice: the deploy builds, the gate, `plan:verify`), and returns `ok` or a `failed` reason (`no-entry`, `import`, `timeout`, `crashed`, `old-server`). The child checks `Application.prototype.introspect` on the `@guren/core` the entry resolves *before* importing the entry, since a scaffolded `src/main.ts` boots at import and an older server would run the real boot; it resolves controller identity through `discoverControllerFiles()`, so server never restates the discovery rule, and writes its result to a temp file because app modules print to stdout. Type-only against `@guren/server`: the app may resolve an older one |
