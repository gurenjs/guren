...
| `packages/cli/src/plan/command-allowlist.ts` | The one rule for a plan's `commands` (RFC 0030 §8): `guren <subcommand>` or `bunx guren <subcommand>`, unquoted, then arguments tokenized on spaces and `'`/`"` quotes over a closed character set (letters of any script, digits, `_-.,:/=@+%`, and a space or `?` inside quotes), so an escape, a shell metacharacter, a control or invisible character, or an unbalanced quote refuses the command. `PLAN_COMMAND_CLASSES` classifies every registry command, `add`'s blueprints as `add <name>`: only generators (`make:*` but `make:migration`, `lang:publish`, `add <blueprint>` but `add plugin`) pass, and a name it does not list is refused. `tests/plan-command-allowlist.test.ts` fails on a registry command it does not classify and on a classified name the registry lacks. `validate.ts` (a failing §2 check, so `plan:approve` refuses), `plan:next` (refuses before reading or marking, drafts included) and `check --plan` (advisory, for plans approved before) all judge through `judgePlanCommand()` |
