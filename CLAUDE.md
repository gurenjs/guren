...
| `packages/cli/src/import-resolution.ts` | The one rule for which file a relative or `@/` import lands on: the source ahead of a stale emitted twin, the six extensions, a directory's `package.json` `main` (after `types`/`typings` for a type-only import; `module` and `exports` are not read, as Bun and TypeScript read neither for a relative import) and then its index, never the directory itself, and a `.d.ts` only for a type-only import. `guren check --arch` and the route-wiring check both resolve through it; a second resolver is how `'../modules/billing'` resolves to a directory no rule matches. The arch check's module surface is what `resolveDirectoryImport()` gives for a module's directory and its `db/schema`, wider than `moduleDescriptorCandidates()` (`index.ts`/`index.js`), which descriptor reading still uses |
