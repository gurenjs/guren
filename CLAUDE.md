...
bunx guren plan "comments on posts" --print-prompt  # The prompt and the draft JSON Schema (draft-07) an agent writes a plan from (RFC 0030 §8), for an agent already in a session: the request (or an instruction to ask for one), the read-only commands to read the app with (named, never embedded), the rules (questions as data, no `baseline`, ids by section, every `alter` in properties plan:status reads, `AC-` behaviours, `commands` from the allowlist's generators), and the `plan:render --json` loop; calls no model and spawns nothing. --json prints { prompt, schema }. Without --print-prompt it exits 1, since the headless producer is deferred; --revise exits 1 naming plan:revise
...
| `packages/cli/src/plan/prompt.ts` | `buildPlanPrompt()`, the producer prompt of RFC 0030 §8 and the one the deferred headless producer is to call: pure, returning `{ prompt, schema }` with `planDraftJsonSchema()` as the schema. It embeds no application context and names the read-only commands that print it instead, so it imports no app code and needs no size bound. The allowed `commands` are listed from `PLAN_COMMAND_CLASSES`, so the prompt and the §2 check cannot disagree; `tests/plan-prompt.test.ts` holds each rule to a phrase, and every `bunx guren` invocation in the prompt to a registered command and its declared flags |
