...
| `packages/cli/src/plan-next.ts` | `guren plan:next` (RFC 0030 §7): the first step in task order whose record does not stand, with what it covers and nothing of the rest, and the mark (`active` in the state file) the Stop hook reads. Runs nothing and never loads the application, which is why it can refuse a dirty tree cheaply; one step is one commit, so the only uncommitted work it accepts is the marked step's own, the state files excluded by pathspec. The freshness skip of §4 (`freshness.ts` judges it; nothing blocks on it yet) and the entity context are not in it yet |
...
| `packages/cli/src/plan/freshness.ts` | `stampContextHash()` and `judgeFreshness()` (RFC 0030 §4): one sha256 per element over the facts its targets read (which app roots declare the name, a table's identity without its columns, whether a column's table declares it, a route name's endpoints), across every app root. A section that cannot be read stamps no entry and judges `unjudged`, never `fresh`; an element with no entry is `unstamped`. A differing hash is `stale` for an `existing` element always, and for any other only while `plan:status` holds no evidence the plan's own work reached it (`planned`, `blocked`, `unjudged`). The stamp is part of `planHash()`, so changing what it hashes renames every approved plan |
