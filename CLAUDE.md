...
bunx guren plan:scaffold comments.plan.json --step task/entity/model.comment/scaffold  # Write a scaffold step of an approved plan (RFC 0030 §5), the one plan:next marked and names this command for: per model the step adds, its table appended to the root db/schema.ts through appendTableToSchema() in the schema's dialect (every column option plan:status compares: type, nullable, unique, index, default, columnName, withTimezone, precision/scale, primary key, foreign key with onDelete, and the model's multi-column indexes) and app/Models/<Name>.ts with the plan's fillable and relationships, keyed by the foreign keys the plan states (one whose target or keys do not exist yet is left out and listed). Writes nothing else today: validators, resources, policies, controllers and routes stay the http step's, and pages are never emitted. Runs no codegen and no migration. Refuses, before any write, a draft or an unapproved plan, another step kind, an unmarked step, an element in a module (root only), an API-only app, a foreign key to a table not declared yet, a MySQL key over a text or json column (primary, unique, index, or a foreign key's), a `default` of null, and any target that exists (model file or class, schema export, the table name in any root), which is also how a re-run of a scaffolded step is refused (--app <dir>, --json)
...
| `packages/cli/src/plan/scaffold.ts` | The plan-driven scaffold emitter (RFC 0030 §5, Part 3 item 6), pure: `emitPlanScaffold(plan, step, app)` from a scaffold step and the facts the command reads (the root schema's dialect, every root's tables, the root's model classes) to table blocks, model files, refusals and the relationships left out. Every planned property is written in a form `plan/status.ts` reads back, and the round trip in `tests/plan-scaffold.test.ts` holds it there per dialect, the readers' own limits listed (`references.onDelete` everywhere; SQLite's moded `integer`/`text`, sizeless `numeric` and `unixepoch()` default; MySQL's `varchar` uuid and `withTimezone`). `planScaffoldCoverage()` is the one rule for which of a step's `generates` it writes (each added model and its added columns) and which it leaves to the `http` step: its report and `plan:next` both say it through there, so a later emitter extends that function. A relationship's keys come from the foreign keys the plan states, its target's record type is a local alias over the target's table (the name `model-parser.ts` reads the target by), and a self-reference goes to the extra config as `foreignKey()` (TS7022) |
| `packages/cli/src/plan-scaffold.ts` | `guren plan:scaffold`: the approval gate (drafts refused too), the step and mark checks, the reads the emitter needs (`parseSchemaTables()`, `discoverModelClasses()`, never the runtime reader), then every refusal before the first write: the emitter's, and each model file already on disk. Writes the schema first, through `appendTableToSchema()` (the model files import its exports), then each model file through `writeScaffoldFiles()`; a write failing after the first names what is already on disk. Runs no codegen or migration |
| `packages/cli/src/schema-columns.ts` | The per-dialect drizzle column builders keyed by plan column type (`COLUMN_BUILDERS`), `autoIncrementPrimaryKey()`, and the `--fields` mapping `guren add resource` appends through (`buildFieldColumn()`, whose MySQL `text` stays `varchar(255)`). `plan:scaffold` and the resource blueprint both write columns here, so a dialect rule lives once; `tests/blueprints.test.ts` pins the blueprint's tables byte for byte, and a builder changed here must stay one `COLUMN_TYPE_BUILDERS` in `plan/status.ts` accepts for the type |
