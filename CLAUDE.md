...
| `packages/cli/src/plan-next.ts` | `guren plan:next` (RFC 0030 §7): the first step in task order whose record does not stand, with what it covers and nothing of the rest, and the mark (`active` in the state file) the Stop hook reads. Spawns nothing; a draft never has the application read, an approved plan has it read without `detail` for the freshness skip of §4 (`plan/stale-steps.ts` decides what blocks), and a load that throws blocks nothing. A blocked step holds the rest of its task and every task waiting for it. One step is one commit, so the only uncommitted work it accepts is the marked step's own, the state files excluded by pathspec. The entity context is not in it yet |
