...
| `packages/cli/src/plan/field-readers.ts` | The two field readers behind `plan:status` (RFC 0030 §6), judged by the pure `field-status.ts`. A validator's fields are the JSON Schema walker's reading (`toJsonSchema()` per key on both sides, `readObjectSchema()` for presence, from `@guren/server/internal/zod-json-schema`, the one agent tools and OpenAPI use) of the export `app-detail.ts` imports, so a schema built with `.extend()` or a helper reads like a literal one. The type is judged on the output side, since the plan states the validated value, and is `unknown` behind a `.transform()`, which the walker renders as its input; a field with a pipe never differs on type, and one with a transform stage never on `required`. A resource's payload is codegen's own reading (`readResourceDefinitions()` in `data-types.ts`, what `data.gen.ts` is emitted from), parsed into members; a payload codegen references rather than copies, or whose heritage is anything but one `Record<string, …>`, leaves an absent field `unknown`. `differ` only where both readings cannot hold: a key not declared, another JSON family, a stated bound tighter than planned, a literal against a keyword it is not |
