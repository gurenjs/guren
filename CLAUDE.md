...
| `packages/cli/src/test-requests.ts` | Which routes the existing tests request (RFC 0030 §2): `TestApp` calls (`get`/`post`/`put`/`patch`/`delete`/`query`, the GET `withCsrf()` makes, `agent().call('tool')`) read by AST, so a comment or string is none, on a receiver the file itself types as a `TestApp` (annotation, factory, builder, a same-file function annotated to return one), matched by name and never across files. A path the file does not spell is unresolved with its line, never guessed; a whole-segment interpolation matches a route parameter and never a literal segment. Route patterns are compared locally, since `@guren/cli` does not depend on hono. Impact lists the test files reaching a route beside the file-name matches, which stay: `examples/blog` and `examples/api` test controllers without HTTP, so the scan finds no request there |
