...
| `packages/cli/src/plan/impact.ts` | Impact (RFC 0030 §2), pure: per element whose change is `alter`/`rename`/`drop`, the consumers the static readers find (relationships, bound routes and their `ApiRoutes` entries and agent tools, resources, policies, actions naming it, tests by file name) and, for a column, its reads from `column-consumers.ts`, the AST scan of controllers, resources and pages that follows a record only from where the file ties it to the model. Every list is a lower bound and the page says so; `impact-sources.ts` builds the input under `loadPlanAppState({ impact: true })`, which imports no `db/schema.ts`. The breaking rule stays `planBreakingChanges()`; `impactBreakingChanges()` adds only an altered route or action whose app route publishes a tool the plan does not declare |
