...
| `packages/cli/src/sessions-check.ts` | AI agent: session wiring checks (RFC 0020 §2) — a `database` store bound to a table the schema does not export, and a `SessionConfig` no provider binds as `session` (the config is then never read and sessions stay in process memory). Content-activated: an app with no session config contributes nothing |
