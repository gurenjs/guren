...
| `packages/cli/src/plan/field-readers.ts` | The two field readers behind `plan:status` (RFC 0030 §6), judged by the pure `field-status.ts`. A validator's fields are the JSON Schema walker's reading (`toJsonSchema()` per key on both sides, `readObjectSchema()` for presence, from `@guren/server/internal/zod-json-schema`, the one agent tools and OpenAPI use) of the export `app-detail.ts` imports, so a schema built with `.extend()` or a helper reads like a literal one. The type is judged on the output side, since the plan states the validated value, and is `unknown` behind a `.transform()` (the field's or the object's), which the walker renders as its input; a field with a pipe never differs on type. `required` is `unknown` where a stage may fill a missing value (a transform, `.default()`, `.prefault()` or `.catch()` anywhere in the chain), where a refinement may require an omissible key, and where the object pipes into a second object; a key the walker dropped unrendered (`z.file()`) is read off its outermost presence wrapper, so `.required()`'s `nonoptional` over `.optional()` is not `false`. A bound is read only in the planned type's unit, and none behind a transform. A walk that throws (a recursive getter schema) leaves the export unread. A resource's payload is codegen's own reading (`readResourceDefinitions()` in `data-types.ts`, what `data.gen.ts` is emitted from), parsed into members; a payload codegen references rather than copies, or whose heritage is anything but one `Record<string, …>`, leaves an absent field `unknown`. `differ` only where both readings cannot hold: a key not declared, another JSON family, a stated bound tighter than planned, a literal against a keyword it is not |
