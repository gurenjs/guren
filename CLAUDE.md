...
bunx guren plan:revise comments.plan.json --edited comments.edited.json --message "soft-delete comments"  # A revision with no model (RFC 0030 §4): the plan file as it stands is the parent, the change an edited copy (ops from diffPlans(), --message their reason, --reopens on each) or --ops <file> ({ "ops": [...] }, each with its own reason); `-` reads standard input for one of them. Stamped by createPlanRevision(), recorded as { parent, ops, result } in revisions/<n>.json beside a docs/plans/<slug>/plan.json (else <slug>.revisions/), then the plan file is rewritten to the result (the copy verbatim, or the result keeping the author's omitted sections); a draft revises under planDigest(). --feedback <file|-> applies the page's approvals (reopens required) and answers (the question removed), never its comments. Refuses a plan with a baseline whose hash no approval or revision record names (edited in place: restore it and pass the edit as --edited), and names the waivers the parent hash held, which do not carry over (--app <dir>, --json)
...
bunx guren plan:scaffold comments.plan.json --step task/entity/model.comment/scaffold  # Write a scaffold step of an approved plan (RFC 0030 §5), the one plan:next marked and names this command for: per model the step adds, its table appended to the root db/schema.ts through appendTableToSchema() in the schema's dialect (every column option plan:status compares: type, nullable, unique, index, default, columnName, withTimezone, precision/scale, primary key, foreign key with onDelete, and the model's multi-column indexes) and app/Models/<Name>.ts with the plan's fillable and relationships, keyed by the foreign keys the plan states (one whose target or keys do not exist yet is left out and listed); the step's validators in app/Http/Validators/<Model>Validator.ts (a query or params validator coerces numbers and booleans; a prose rule is listed, not written); each resource of the model with its planned payload (a field no column reads back as is calls a stub that throws, listed); each policy of the model, every ability `return false` until written, and app/Providers/<Policy>Provider.ts calling `gate.policy()` in boot(), registered in createApp() by `wireAppProvider()`'s patch, composed before any write and applied last. Controllers and routes stay the http step's, and pages are never emitted. Runs no codegen and no migration. Refuses, before any write, a draft or an unapproved plan, another step kind, an unmarked step, an element in a module (root only), an API-only app, a foreign key to a table not declared yet, a MySQL key over a text or json column (primary, unique, index, or a foreign key's), a `default` of null, a resource name not ending in Resource, an ability named after a `Policy` member, a provider it cannot register (no entry, no patchable createApp(), already registered), and any target that exists (model file or class, schema export, the table name in any root, a file it would create, a validator name a root validator file exports, a resource or policy class of that name), which is also how a re-run of a scaffolded step is refused (--app <dir>, --json)
...
| `packages/cli/src/app-routes.ts` | The join of the introspected app's routes to the routes file's definitions that the Part 2d consumers read (RFC 0026 §5), on method, path, name and controller action, nth to nth, and nothing where the two sides count a key differently. `loadIntrospectedRouteDefinitions()` is what `guren context` and `codegen --introspect` read: the manifest decides which routes exist and in which order, and each joined route is the routes file's definition, whose Zod every renderer reads; an unmatched route (a provider's) comes from the manifest without schemas. `route-contract-check` joins through it for its Zod fallback, and `attachControllerRefs()` in `controller-methods.ts` through `joinRouteDefinitions()`, so a manifest reference reaches a registered definition by the same key and ambiguity rule. The key holds each side's `module` too (the `defineModule()` name, which `mountModuleRoutes()` records on each `RouteDefinition` it registers and the manifest's `RouteEntry` carries from there), so a key two modules share never pairs by module order, which the CLI (directory order) and the app (`createApp({ modules })` order) need not share. `loadRouteDefinitions()` names the module itself where an older `@guren/server` left it off (`withModuleNames()`), so a definition it returns always carries its module. A module the app mounts under another `defineModule()` name than its `modules/<dir>/index.ts` exports joins none of its routes. Codegen stays on the routes file by default, and so does `planAgentManifest()`, the rule for what codegen writes: `--introspect` output is one-shot, and a `.guren/agents.gen.ts` it wrote from provider-only tools reads as stale to `check` and `doctor`. A tool name two routes claim goes to the first registered, as `deriveAgentTools()` does at runtime |
...
| `packages/cli/src/import-resolution.ts` | The one rule for which file a relative or `@/` import lands on: the source ahead of a stale emitted twin, the six extensions, a directory's `package.json` `main` (after `types`/`typings` for a type-only import; `module` and `exports` are not read, as Bun and TypeScript read neither for a relative import) and then its index, never the directory itself, and a `.d.ts` only for a type-only import. `guren check --arch` and the route-wiring check both resolve through it; a second resolver is how `'../modules/billing'` resolves to a directory no rule matches. `moduleEntryFile()` is a module's entry by that rule (its descriptor, and with its `db/schema` the arch check's module surface), read by descriptor reading, route loading, the console check, `make:command` and `make:feature`; `moduleRoutesEntryFile()` is its `./routes` import, and `MODULE_ENTRY_FILE`/`MODULE_ROUTES_FILE` the names `make:module` writes and every "create this file" hint falls back to |
...
| `packages/cli/src/plan-revise.ts` | `guren plan:revise` (RFC 0030 §4, Part 3), the model-free revision: the plan file as it stands is the parent, accepted as a draft nobody approved, at an approved hash (`readPlanApprovalStanding()`), or at the `result` of a record beside it, so a plan can be revised again before approval; anything else is an edit made in place and refused with the remedy. Ops come from `--ops` or from `diffPlans()` over `--edited`, read through the feedback's counting reader (`readJsonWithinLimit()`), and are stamped by `createPlanRevision()`, which enforces the feedback's locks and answers. The plan is written only after its content reads back at the revision's `result`, and after the record. The records are `plan/revision-records.ts` (numbered `<n>.json`, linked into place so none is overwritten) in the directory `planRevisionsDir()` in `plan/beside.ts` names, which `planBesideExclusions({ records: true })`, the step work measurement and plan discovery all know |
...
| `packages/cli/src/plan/scaffold.ts` | The plan-driven scaffold emitter (RFC 0030 §5, Part 3 item 6), pure: `emitPlanScaffold(plan, step, app)` from a scaffold step and the facts the command reads (the root schema's dialect, every root's tables, the root's model, resource and policy classes and validator exports) to table blocks, model, validator, resource, policy and provider files, the providers to register, refusals, the relationships left out and what it wrote as a stub or not at all (`unwritten`). Every planned property is written in a form `plan/status.ts` reads back, and the round trip in `tests/plan-scaffold.test.ts` holds it there per dialect, the readers' own limits listed (`references.onDelete` everywhere; SQLite's moded `integer`/`text`, sizeless `numeric` and `unixepoch()` default; MySQL's `varchar` uuid and `withTimezone`). `planScaffoldCoverage()` is the one rule for which of a step's `generates` it writes (each added model and its added columns, the validators, and each resource and policy whose model the step adds, a resource whose payload type names an import excepted) and which it leaves to the `http` step: its report and `plan:next` both say it through there, so a later emitter extends that function. A relationship's keys come from the foreign keys the plan states, its target's record type is a local alias over the target's table (the name `model-parser.ts` reads the target by), and a self-reference goes to the extra config as `foreignKey()` (TS7022) |
| `packages/cli/src/plan-scaffold.ts` | `guren plan:scaffold`: the approval gate (drafts refused too), the step and mark checks, the reads the emitter needs (`parseSchemaTables()`, `discoverModelClasses()`, the root validator files' exports by AST through `exportedNames()`, the resource and policy discovery; never the runtime reader, and a validator file whose exports it cannot read refuses), then every refusal before the first write: the emitter's, each file already on disk, and the provider registration, composed in memory by `composeAppProviderRegistration()` (`wireAppProvider()`'s patch as a pure function, since that one writes as it goes and only warns). Writes the schema first, through `appendTableToSchema()` (the model files import its exports), then each file through `writeScaffoldFiles()`, then the app entry; a write failing after the first names what is already on disk. Runs no codegen or migration |
| `packages/cli/src/plan/scaffold-http.ts` | The validator, resource and policy half of the plan scaffold, pure. A validator field is written from `TYPE_LEAVES` (a `decimal` as a number, so its bounds read; `json` as `make:validator`'s record, which `field-readers.ts` leaves opaque) and the rules `field-status.ts` parses (`BOUND_RULE`, `FORMAT_RULES`, imported so the writer and the judge read a rule alike); a query or params validator coerces. A resource payload is written verbatim when its type names nothing to import, and a field is copied where the planned type admits every value `COLUMN_RECORD_TYPES` in `schema-columns.ts` says the column reads back as (a `Date` serialized for a planned `string`, a JSON column's `unknown` cast to the planned type), else a throwing `unmapped()` stub. A policy denies every ability until written; its provider is the blog `AuthorizationProvider` shape. The shells are `zodObjectExport()`, `buildResourceSource()` and `buildPolicySource()`, factored out of `make:validator`, `make:resource`/`make:feature` and `make:policy`, whose output `tests/generator-bytes.test.ts` pins byte for byte |
| `packages/cli/src/schema-columns.ts` | The per-dialect drizzle column builders keyed by plan column type (`COLUMN_BUILDERS`), `autoIncrementPrimaryKey()`, and the `--fields` mapping `guren add resource` appends through (`buildFieldColumn()`, whose MySQL `text` stays `varchar(255)`). `plan:scaffold` and the resource blueprint both write columns here, so a dialect rule lives once; `tests/blueprints.test.ts` pins the blueprint's tables byte for byte, and a builder changed here must stay one `COLUMN_TYPE_BUILDERS` in `plan/status.ts` accepts for the type, and keep `COLUMN_RECORD_TYPES` (the record type drizzle gives it, which the plan scaffold maps resource fields by) true |
...
| `packages/cli/src/plan/prompt.ts` | `buildPlanPrompt()`, the producer prompt of RFC 0030 §8, pure so the deferred headless producer can call it. Its `commands` rule lists `planGeneratorNames()`, so the prompt and the §2 check cannot disagree |
