...
| `packages/cli/src/plan/verification.ts` | The overlay both `plan:status` and `plan:verify` report through, so they cannot disagree: `applyVerification()` is pure and, given the records and what the files hash to now, lifts to `verified` or `drifted`; a record of another plan digest is stale, and an element none of whose files the record covers is never lifted, since that result could not expire. A `drop` has no file and lifts on the step alone; an `unjudged` element lifts only from a step with behaviours, whose test files the record covers. `overlayVerification()` is the one reading of `.guren/plans/`. It executes nothing, which is what keeps `plan:status` from importing `check.ts` |
