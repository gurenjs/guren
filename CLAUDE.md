...
| `packages/cli/src/plan/field-readers.ts` | The two field readers behind `plan:status` (RFC 0030 §6), judged by the pure `field-status.ts`. A validator's fields are the JSON Schema walker's reading (`readObjectSchema()` from `@guren/server/internal/zod-json-schema`, input side, the one agent tools and OpenAPI use) of the export `app-detail.ts` imports, so a schema built with `.extend()` or a helper reads like a literal one; an export that reaches no object (`z.lazy()`, a plain object, a file that would not import) leaves every field `unknown`. A resource's payload is codegen's own reading (`readResourceDefinitions()` in `data-types.ts`, what `data.gen.ts` is emitted from), parsed into members; a payload codegen references rather than copies, or whose type `extends` anything but `Record<string, …>`, leaves an absent field `unknown`. `differ` only across JSON families (a string where the plan says boolean), on a bound both sides state (`max 2000` against `maxLength` 500) and on primitive resource types; a union, a format or refinement the walker does not render, and an alias or object type read as text stay `unknown` |
