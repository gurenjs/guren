...
| `packages/cli/src/spec-generate.ts` | AI agent: spec view orchestration (`spec:generate`, RFC 0004). Reads the routes file only, never the introspected app: the views are committed, and `guren gate` and the edit hook regenerate them in process without introspecting (RFC 0026 Part 2d) |
...
| `packages/cli/src/deploy-runtime.ts` | The one deploy-runtime reading *and* its three verdicts (`judgeDeployRuntime`, `checkDeployRuntime`): in-memory stores, a Bun-only hasher, filesystem provider discovery on Workers/Lambda/Vercel. `guren doctor` maps them onto its checks, `guren check` onto advisory results, and the three deploy builds print them through `@guren/core/internal/deploy-check` (RFC 0020 Part 0), so one app cannot get three different answers. Once a deploy target is found it reads the introspected app (RFC 0026 §5, one `introspectApp()` per run): the auth manager's hashers by `requiresBun`, the selected session store's `perProcess` (a plugin driver's from its `gurenPlugin` manifest), a `memory` cache default. A section `manifest-section.ts` calls unverified falls back to the scan with `evidence: 'static'` and the reason; only whether the cache is per-process, which has no scan, becomes `deploy-runtime-stores-unverified` with `evidence: 'none'`. What the manifest does not carry stays on the source scan: target detection, provider discovery (a discovered provider registers as `app.register`, like an explicit one), OAuth state stores, the queue (`QueueManager.describe()` knows no driver), explicit `Memory*Store` constructions, `autoSession: false`, a `sessionOptions.store` factory, and password hashing when no user provider is registered yet the source hashes (a `useModel()` in `boot()`). The scan is also the whole fallback when introspection fails or `--no-introspect` is passed. Only `checkDeployRuntime(cwd)` introspects by default (capped at 10 s for the deploy builds); `analyzeDeployRuntime()`, `runCheck()` and `runDoctor()` need `introspect` passed (the commands do), so the edit hook, the gate and the dev MCP server never hold a stale manifest. The scan reads *constructions* plus a session config's selected `driver:` (read by `session-config.ts`); it never reads intent. The manifest is read with the local `.env`, so every verdict stays advisory |
| `packages/cli/src/manifest-section.ts` | The one reading of RFC 0026 §5's fallback rule for a manifest section, which every check reading the introspected app goes through: described, or unverified when a provider threw (any section, since `auth` is always bound), a config was left unbound for an unset env key (`config-unverified` with its `key`, checked before the value since an unbound `session` still reads as `source: 'none'`), a deferred provider supplies it, or it is bound with no `describe()`. Stricter than the server's `readSection()` on purpose. A check with a static path falls back to it for an unverified section; only a fact with none becomes a `<key>-unverified` warn with `evidence: 'none'`, never a pass. `introspectedSection()` is the form such a check calls: it asks for the run's introspection only then, so the check's own content is what starts the child, and hands back the section or the reason to judge from source |
| `packages/cli/src/schema-check.ts` | AI agent: Postgres `timestamptz` schema checks (part of `guren check`) |
| `packages/cli/src/session-config.ts` | The one rule for reading a `SessionConfig` out of source (RFC 0020): which declarators are session configs (annotation *or* `satisfies`), plus the object a `defineSessionConfig()` resolver returns (RFC 0027 §2), what `default` selects (through `??`/`||`), and each store's `driver`. `guren check`'s session rules and the deploy-runtime verdicts both read it — a second reading is how one reports a backed store while the other skips the table it binds. Both read the introspected app first (RFC 0026 §5); this reading is what activates the session rules, maps a manifest store back to the config and export that declare it, and is the static fallback. The anchor is the *type*, never the file name: a cache config keys `default`, `stores` and `driver` identically, and `createSessionManager(config)` carries no literal since the scaffold passes the config by name across modules. `DEFAULT_SESSION_STORE_NAME` and `PER_PROCESS_SESSION_DRIVERS` come from the runtime rather than being restated here |
...
| `packages/cli/src/sessions-check.ts` | AI agent: session wiring checks (RFC 0020 §2) — a `database` store bound to a table the schema does not export, and a `SessionConfig` whose binding provider `createApp()` does not register (the config is then never read and sessions stay in process memory — the file existing is not the question). Content-activated: an app with no session config contributes nothing, and introspects nothing. Judged from the manifest first: `session.source` (`manager` binds it; `none`, no section, or `auth.sessionOptions.store` means the config is never read) and each `database` store's table by SQL name against the tables the schema declares, keyed as the source reading keys them. A SQL name the reader does not find keeps the source verdict when the source traces the table to a schema export, and is otherwise an advisory warn, never a fail. A `session-configured-twice` warning is a `sessions-binding` fail. A config the app does not read keeps the source reading for its tables |
