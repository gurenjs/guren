...
| `packages/cli/src/authorization-audit.ts` | `guren audit`'s policy rule: a mutating controller route whose action body names a model with an `app/Policies/<Model>Policy.ts` (the name `make:policy` writes, never the `gate.policy()` binding) must show `this.authorize()`/`this.can()`, the policy class, or an `authorization` middleware capability; an unread body is a warn, never a pass. Advisory throughout, content-activated on the first policy, and the one place `GUEST_PATH_PATTERN` lives, which the authentication rule in `audit.ts` shares |
