...
| `packages/cli/src/plan-check.ts` | `guren check --plan` (RFC 0030 §8), run only under that flag, since judging a plan imports `db/schema.ts` (`readSchemaTables`) and every validator file (`readSchemaIdentities`), which `check`, `gate`, `plan:verify`'s check step and the dev MCP must not pay for. The one plan discovery rule (the root's `*.plan.json`, and `plan.json` / `*.plan.json` under `docs/plans/`, `revisions/` skipped) and `isPlanInput()` for `--changed`; open = approved at the current hash (`approvalAt`) and not closed (`planDocClosedHash()` of `docs/plans/<slug>.md`); drift as `planStatusFile()` reports it; overlap keyed on `listPlanAppTargets()` names, never ids, tables app-wide, a changed column or action also claiming a parent another plan renames or drops. Every result is an advisory warn |
