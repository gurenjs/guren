...
/** Where the `local` disk holds one attachment's objects, from its delivery URL. */
...
async function createPost(page: Page, title: string, options: { cover?: boolean } = {}): Promise<string> {
  ...
  // store() redirects to the show page; click through from the index so callers
  // always get the post's own URL.
...
test.describe('Posts — authenticated CRUD', () => {
  ...
  test('create a post with a cover image', async ({ page }) => {
    ...
    // On the private disk, so the show page renders it through the signed route.
    const cover = page.getByTestId('post-cover')
    await expect(cover).toBeVisible()
    await expect(cover).toHaveAttribute('src', /^\/attachments\/[^/]+\/.*[?&]signature=/)

    // A broken image keeps naturalWidth at 0 even though the element is "visible".
    await expect
  ...
  test('delete a post removes it and its attachments', async ({ page }) => {
    ...
    // Asserted before deleting, so the "gone afterwards" check below cannot pass
    // by pointing at a path that never existed.
    const storedObjects = storageDirFor(coverSrc!)
    expect(existsSync(storedObjects)).toBe(true)

    // The confirm dialog only exists once React has hydrated.
    await gotoHydrated(page, `${postUrl}/edit`)
    ...
    // Asserted separately: the delivery route 404s on the missing row before it
