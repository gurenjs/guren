...
export default defineConfig(({ command }) => ({
  ...
  plugins: [
    // Dev-server only: the plugin re-running the codegen script during
    // `vite build` would overwrite the real prerender output with stubs (the
    // script runs `prerender:stub`).
