...
import { COLOR_MODE_STORAGE_KEY } from '../../config/theme.js'
 ...
 *
 * - The color-mode toggle is a few lines of inline vanilla JS sharing
 *   `COLOR_MODE_STORAGE_KEY` with the React implementation, so the
 *   preference round-trips between Inertia and content pages.
 * - The mobile menu is a `<details>` disclosure instead of React state.
 */

const COLOR_MODE_TOGGLE_SCRIPT = `(function(){
...
export const Header: FC<{ basePath?: string }> = ({ basePath = '/docs' }) => (
    ...
    <div class="mx-auto flex max-w-[1280px] flex-wrap items-center justify-between gap-4 px-6 py-3">
      ...
      <div class="flex flex-wrap items-center gap-5">
        <nav class="hidden items-center gap-6 text-[0.95rem] font-medium md:flex">
          <a href={basePath} class="text-docs-text no-underline transition hover:text-docs-accent">Guides</a>
          <a href={`${basePath}/tutorials/overview`} class="text-docs-text no-underline transition hover:text-docs-accent">Tutorials</a>
          <a
        ...
        <details class="relative md:hidden">
          ...
          <nav class="absolute right-0 mt-3 flex w-48 flex-col gap-1 rounded-xl border border-docs-border bg-docs-panel p-3 text-[0.95rem] font-medium shadow-lg">
            ...
            <a href={basePath} class="rounded-md px-3 py-2 text-docs-text no-underline transition hover:text-docs-accent">Guides</a>
            <a href={`${basePath}/tutorials/overview`} class="rounded-md px-3 py-2 text-docs-text no-underline transition hover:text-docs-accent">Tutorials</a>
            <a
              href="https://github.com/gurenjs/guren"
              target="_blank"
              rel="noreferrer"
