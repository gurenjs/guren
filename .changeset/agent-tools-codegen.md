...
- `guren check` and `guren doctor` account for `.guren/agents.gen.ts` conditionally: an app whose routes declare no agent metadata is not missing a manifest, and is reported as such rather than warned at.
