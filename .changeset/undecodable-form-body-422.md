...
Two things this deliberately does not change. An all-optional object schema keeps *passing* on an undecodable body, because the fallback is `{}` and not `undefined` — the same answer it has always given for an empty POST or malformed JSON. And it covers the paths that hand the body to a schema, not every read of one: `Controller.file()` / `files()` parse the multipart body themselves and still surface the parser's error, while CSRF token extraction catches it and fails closed at 403, both unchanged.
