...
While a deploy runs this release beside the previous one, the two do not fully exclude each other on a key: a previous-release process can reject `increment()` or `add()` with `ENOTEMPTY` when it meets a lock this release holds, either after its update (a lock taken over from it) or before (its own five-second deadline).
