...
**Follow-up for whoever cuts the release after this one.** The client entry needs `@guren/core/agent`, which arrives in core 1.13.0 — but this package ships declaring `@guren/core: ^1.12.0` and `gurenPlugin.compatibility: ">=1.12.0 <2.0.0"`, because `audit:plugin-compat` checks both against the *workspace* core version, and that is 1.12.0 until `changeset version` runs. So an app resolved at core 1.12.0 satisfies both claims, installs cleanly, and then fails on a missing export path instead of getting the clean compatibility refusal `guren plugin` is supposed to give. Nothing forces the correction on its own either: `^1.12.0` still admits 1.13.0, so the audit stays green indefinitely. **Once the release publishing core 1.13.0 has shipped, bump the dependency to `^1.13.0` and `compatibility` to `">=1.13.0 <2.0.0"`.**
