...
- `guren context` prints the installed Guren version (the `@guren/server` release the app runs) instead of the `@guren/core` range from `package.json`, which sits on its own version line.
