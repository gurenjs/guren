...
It warns (`ai-agent-plugin-missing`) when no source file calls `aiPlugin()`, and when two files declare an agent under one class name (`ai-agent-name-collision`). A spread, a variable or a computed element in `appTools()`, and a non-literal `static scopes`, are reported as unverifiable warnings rather than passed.
