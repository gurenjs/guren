...
- 55137f7: Run the App MCP endpoint on the framework's invocation pipeline, and
...
- e94645b: Read the approval-status rule from `@guren/core` instead of restating it
...
- 59347c1: Answer `401` rather than a CSRF `403` when the App MCP endpoint is called
