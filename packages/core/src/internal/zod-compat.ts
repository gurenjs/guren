 ...
 * door (`zod-json-schema.ts`, behind both `@guren/openapi` and RFC 0016's agent
 * tools), `@guren/cli`'s TypeScript-type renderer (`src/schema-type-extractor.ts`),
 * and its route contract check (`src/route-contract-check.ts`).
 *
