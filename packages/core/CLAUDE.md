...
## Purpose
Recommended public entrypoint for application code. Re-exports the stable framework surface from `@guren/server` + `@guren/orm`. Exposes the `guren` CLI by proxying to `@guren/cli` (`src/bin.ts`). Re-exports Bun/dev helpers through `@guren/core/runtime` and `@guren/core/vite`.
