...
- Generator templates come in two forms, chosen by whether the contents depend on flags or fields:
  ...
  - `make:auth`'s builder output additionally gets a *compile* gate: `tests/scaffold-builder-typecheck.test.ts` renders two flag combinations (`--oauth --verify` and `--oauth-only`) into a temp app, regenerates `.guren/pages.gen.ts`, and typechecks the whole render with tsconfig.templates.json's own options — so the static and rendered halves of the scaffold are held to the same bar. Combos not compiled there differ only by omission and stay parse-checked; other generators' builder output is parse-checked only.
