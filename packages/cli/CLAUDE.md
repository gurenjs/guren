...
- For a failed diagnostic/report, call `markCommandFailed()` from `command-status.ts`, never `process.exit()` or `process.exitCode`. It lets remaining output and cleanup finish, and `runCli()` returns 1 without reporting again or changing the caller's `process.exitCode`; each invocation (nested or concurrent included) has its own status, and a helper called outside `runCli()` sets `process.exitCode`. Throw `CliError` for failures that still need a user-facing diagnostic; process termination belongs to the bin and to a long-running command's own shutdown
- `collectFiles()` and `listModuleNames()` tolerate absent optional directories,
  but throw `FileDiscoveryError` for unreadable or malformed paths. Check and
  audit turn that error into a failed, incomplete report without retaining partial
  pass verdicts. Other errors still propagate. Do not replace discovery failures
  with empty results; that would silently remove code from the checks.
