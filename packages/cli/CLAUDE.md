...
- For a failed diagnostic/report, call `markCommandFailed()` from
  `command-status.ts`. It lets remaining output and cleanup finish, and `runCli()`
  returns 1 without reporting again or changing the caller's `process.exitCode`.
  Each invocation has its own async status, including nested/concurrent calls.
  Helpers called directly outside `runCli()` retain their exit-code side effect.
  Throw `CliError` for failures that still need a user-facing diagnostic; reserve
  process termination for the bin and explicit long-running command shutdown.
