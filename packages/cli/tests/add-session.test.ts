...
describe('guren add session', () => {
  ...
  // RFC 0027 §2, and the checks that read sessions keep judging the definition.
  it('installs a config/session.ts definition that guren check reads as wired and backed', async () => {
    await seedApp(PG_SCHEMA_FIXTURE, { env: 'APP_KEY=\n' })
    await writeWorkspaceFiles(process.cwd(), { 'config/env.ts': ENV_SCHEMA_FIXTURE })

    await runBlueprint('session', {})

    expect(existsSync('app/Providers/SessionProvider.ts')).toBe(false)
    expect(await readFile(resolve('config/session.ts'), 'utf8')).toContain('export default defineSessionConfig((env) => ({')
    const app = await readFile(resolve('src/app.ts'), 'utf8')
    expect(app).toContain("import session from '../config/session.js'")
    expect(app).toMatch(/config: \[session\]/)
    expect(await readFile(resolve('src/console.ts'), 'utf8')).toContain('kernel.registerMany([SessionsPruneCommand])')

    const report = await runCheck({ cwd: process.cwd() })
    const sessions = report.checks.filter((check) => check.key.startsWith('sessions-'))
    expect(sessions.map((check) => [check.key, check.status])).toEqual([['sessions-config:config/session.ts:sessions', 'pass']])
