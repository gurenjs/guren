...
const KEY = 'authorization:DELETE /posts/:id'
...
`,
  ...
  it('contributes nothing when the app keeps no policy', async () => {
    ...
    expect(report.findings.some((f) => f.key.startsWith('authorization:'))).toBe(false)
  })
  ...
  it('passes an action that references no model with a policy', async () => {
    ...
    const authorization = report.findings.find((f) => f.key === KEY)
    expect(authorization?.status).toBe('pass')
    expect(authorization?.message).toContain('references no model')
  })
...
`,
  ...
  it('reads neither a model name in a string nor an authorize() in a comment', async () => {
    ...
    expect(stringOnly.findings.find((f) => f.key === KEY)?.message).toContain('references no model')
  })
  ...
  it('does not read Post inside PostTag as a reference', async () => {
    ...
    expect(report.findings.find((f) => f.key === KEY)?.message).toContain('references no model')
  })

  it('honours // guren-audit-ignore above the action', async () => {
    const report = await withWorkspace(app({
      'app/Http/Controllers/PostController.ts': `import { Controller } from '@guren/core'
import { Post } from '../../Models/Post'
...
`,
    ...
    expect(report.findings.some((f) => f.key === KEY)).toBe(false)
  })
...
`,
    ...
    const authorization = report.findings.find((f) => f.key === 'authorization:DELETE /invoices/:id')
    expect(authorization?.status).toBe('warn')
