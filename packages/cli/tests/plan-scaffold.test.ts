...
import { emitPlanScaffold } from '../src/plan/scaffold'
...
interface AppOptions {
  ...
  document?: PlanInput | Record<string, unknown>
...
async function createApp(name: string, options: AppOptions = {}): Promise<{ dir: string; plan: string }> {
  ...
  const document = options.document ?? approvedAgainst(widgetsPlan() as unknown as Record<string, unknown>)
...
/** Every planned property of what the run wrote that does not read `match`, as `<id> <property>`. */
async function unmatched(dir: string, plan: string, emitted: readonly string[]): Promise<string[]> {
  const status = judgePlan(parsePlanDocument(JSON.parse(await readFile(plan, 'utf8'))), await loadPlanAppState(dir, { detail: true }))
  const elements = status.elements.filter((element) => emitted.includes(element.id))
  expect(elements.map((element) => element.id).sort()).toEqual([...emitted].sort())
  for (const element of elements) expect([element.id, element.state]).toEqual([element.id, 'present'])
  return elements.flatMap((element) => element.properties.filter((property) => property.verdict !== 'match').map((property) => `${element.id} ${property.property}`))
}
  ...
  describe('round trip through the plan:status readers', () => {
    ...
    for (const dialect of ['pg', 'mysql', 'sqlite'] as const) {
      test(`should write a ${dialect} table and model every planned property reads back from, bar the readers' own limits`, async () => {
        ...
        expect((await unmatched(dir, plan, report.emitted)).sort()).toEqual([...READER_LIMITS[dialect]].sort())
        scaffolded.push(dir)
      })
    }
    ...
    test('should write a composite primary key the reader reads as differ, column by column', async () => {
      ...
      const { dir, plan } = await createApp('composite', { document: approvedAgainst(document as unknown as Record<string, unknown>), mark: 'task/entity/model.pin/scaffold', link: true })
    ...
    test('should write output that typechecks, in every dialect', () => {
      expect(scaffolded).toHaveLength(3)
      const rootNames = scaffolded.flatMap((dir) => ['db/schema.ts', 'app/Models/Widget.ts', 'app/Models/Post.ts', 'app/Models/Tag.ts'].map((file) => join(dir, file)))
      expect(checkTypes(rootNames, renderedAppCompilerOptions(scaffolded[0]!))).toEqual([])
    }, TSC_TIMEOUT)

    test('should write the pg table and model byte for byte', async () => {
      const dir = join(ROOT, 'round-pg')
...
`)
  ...
  describe('refusals, each with nothing written', () => {
    ...
    test('should refuse a plan no approval names, as the other gated commands do', async () => {
      const message = await refusal((async () => {
        ...
        return planScaffoldFile(plan, { appRoot: dir, step: STEP })
      })())
    ...
    test('should refuse a model in a module: v1 writes to the project root only', async () => {
      ...
      const message = await refusedWithNothingWritten('module', { document: approvedAgainst(document as unknown as Record<string, unknown>) })
    ...
    test('should refuse a foreign key to a table the schema does not declare yet', async () => {
      ...
      const message = await refusedWithNothingWritten('fk-missing', { document: approvedAgainst(document as unknown as Record<string, unknown>) })
  ...
  test('should leave out a relationship whose target has no model yet, and say so', async () => {
    const document = widgetsPlan()
    ...
    const { dir, plan } = await createApp('omitted', { document: approvedAgainst(document as unknown as Record<string, unknown>) })
    ...
    expect(formatPlanScaffold(report, PLAN_FILE)).toContain('  model.widget gadgets: the application has no Gadget model yet')
  })
