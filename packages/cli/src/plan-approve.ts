...
export async function planApproveFile(planPath: string, options: PlanApproveFileOptions): Promise<PlanApproveReport> {
  ...
  // Read before anything is written, so a log that will not read leaves the plan file untouched too.
  requireReadableApprovals(await readPlanApprovals(path))
  ...
  if (hasBaseline(plan)) {
    ...
    const document = { ...(JSON.parse(await readFile(path, 'utf8')) as Record<string, unknown>), baseline: { rev, contextHash: stamp.contextHash } }
    const parsed = PlanSchema.safeParse(document)
  ...
  const recorded = await recordPlanApproval(path, approval)
  return {
