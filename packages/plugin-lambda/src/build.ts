...
export async function buildLambdaOutput(options: BuildLambdaOutputOptions = {}): Promise<void> {
  ...
  // Baked into the wrapper only, never into `env` (and so never into
  // env.json): Lambda caps function environment configuration at 4KB total,
  // which a real app's manifest alone can exceed — as wrapper code it rides
  ...
  writeFileSync(wrapperPath, renderHandlerModule({ out, entrypoint, env, viteManifest }))
...
function renderHandlerModule(input: {
  ...
  if (input.viteManifest) {
    ...
    // render time, and the function bundle ships no
    // public/assets/manifest.json — so the manifest JSON rides in the wrapper.
    envEntries.push(['GUREN_VITE_MANIFEST', input.viteManifest])
  }
