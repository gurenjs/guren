...
- 002d087: `SoftDeletes` models now run the `deleting` and `deleted` hooks and observers on `delete()` and `forceDelete()`, so a `deleting` callback that returns `false` stops a soft delete as the documentation describes. An observer registered with `observe()` inside a lifecycle callback now applies from the next write rather than joining the one in progress. Model create, update, and delete share one internal lifecycle sequence.
- 002d087: Separate relation fetching, record matching, and nested traversal from Model while preserving scoped queries, batching, transforms, and protected loader overrides.
- 002d087: Keep transaction state with its database connection when the default connection changes. Route model updates and deletes through the same scoped write pipeline as query builders, with shared SQLite, PostgreSQL, and MySQL contract coverage.
...
- 3a2acde: Name the migrations a boot applied
...
- 976bd07: `migrationStatus()` on Postgres, MySQL, and SQLite now reports only a missing
...
- 4831473: Resolve a `file:` URI filename to a path in `createSqliteDatabase()`, instead of concatenating it.

  The sqlite driver `mkdir -p`s the database's parent directory before opening it, and it computed that directory with `dirname(resolve(dbPath))`. `resolve()` has no notion of a URI, so `file:///var/tmp/app.db` is taken as a _relative_ name and the driver prepares `<cwd>/file:/var/tmp/`. Running the framework suite deposited one `file:/var/folders/…/guren-sqlite-XXXXXX/` tree into the repository per run. It is untracked, so `git status` says nothing and no build, typecheck, or test gate reads it; only `git clean -nd` reports it, as `Would remove file:/`.

  This is the same class of defect #438/#440 addressed for connection strings, on the other side of the guard they added. That change narrowed the rejection to the _intersection_ — a scheme with an authority, `file:` excluded — precisely because `file:` is sqlite's own URI scheme and names a file rather than a server. The narrowing is unchanged here. What it left standing is that a legal `file:` URI still has to be _parsed_ before it can be used as a path.
  ...
  **The hot-reload key is fixed with the same resolution,** because it read `resolve(dbPath)` too: under `bun --hot`, `file:///data/app.db` and `/data/app.db` are one database but were keyed as two, so a reload that merely restyled the filename left the first handle open instead of replacing it.
  ...
  Eight regression tests in `packages/orm/src/sqlite.test.ts`, each shown to fail against the implementation it guards. The load-bearing one, `should create the directory the URI names, not one named after the URI`, targets a directory that does not exist yet and asserts both halves — the database file was created _and_ no `file:` tree was — so it reds both for the concatenating version and for the tempting shortcut of skipping the mkdir whenever the filename is a URI. The pre-existing `file://` and `file:local.db` cases gain the absence and _location_ assertions they were missing; asserting where the file lands is what makes them portable, since a host that does not parse URI filenames opens one named literally `file:local.db` and reports success. The rest cover percent-decoding in a directory segment, the `localhost` authority, the rejected query, the ignored fragment, and the hot-reload key.
