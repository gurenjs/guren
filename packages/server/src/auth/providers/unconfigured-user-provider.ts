...
/** The remediation both the thrown message and the boot warning end on. */
export const REGISTER_USER_PROVIDER_HINT =
  'Register one with `auth.useModel(User)` in a service provider (what `guren add auth` ' +
  "scaffolds), or `auth.registerProvider('users', ...)`."

export const NO_USER_PROVIDER_MESSAGE =
  'AuthManager: a login was attempted, but no user provider is registered, so there is ' +
  `nothing to check the credentials against. ${REGISTER_USER_PROVIDER_HINT} ` +
 ...
 * password" forever. One object: the guard is built per request.
 */
export const unconfiguredUserProvider: UserProvider = {
