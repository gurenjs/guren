...
/**
 * Trim entries that fell out of the window, record this request, count what
 * is left, and refresh the expiry — as one atomic step. Returns the count.
 *
 * A pipeline is not a substitute: it saves round trips but Redis still
 * interleaves other clients' commands between its steps, so two concurrent
 * callers could both ZADD before either ZCARD and read the same count.
 * The member is generated by the caller so the script stays deterministic.
 */
