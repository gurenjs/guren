...
- bce6db1: The Vercel build (`bun run vercel:build`) keeps the names of named class and function expressions. Bun's syntax minification dropped the name of one whose body never refers to it, so `register(class SendWelcomeMailJob extends Job {})` reported `.name` as `""` in the bundled function. The bundle now sets `minify.keepNames`, which restores the source name on Bun 1.3.14 and 1.4.2. Class declarations are not affected.
  ...
  The Vercel build also warns when Bun renames an unpinned job, event, notification or agent because another module declares the same top-level class name. One of the two is then bundled as `<Name>2`, picked by import order, and no Bun option prevents it.
