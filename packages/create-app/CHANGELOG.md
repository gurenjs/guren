...
- 9779f63: `--auth` no longer lists `bunx guren add auth` under "Add features:" once the scaffolder has run it, and no longer repeats a users table step that told you to run `bun run db:make` first: `guren add auth` prints its own database steps and generates the migration itself. When the step fails after dependencies are installed, the warning no longer tells you to install them first.
...
- Ship template dependency ranges for this release
  ...
  The scaffold's `@guren/*` ranges are generated from the workspace versions,
  and this release moves `@guren/cli`, `@guren/orm`.
  `changeset publish` only uploads packages whose own version moved, so
  without this bump the updated ranges would sit in the repo and never reach
  anyone running `create-guren-app`.
...
- cf3c244: Ship `.guren/*.gen.ts` files that match what codegen writes
...
- fc01a05: Scaffolded apps resolve services from the container
...
- 3ed49af: Scaffolded apps depend on `typescript@^7` (the native compiler). `bun run typecheck`, and the typecheck stage of `guren gate`, drop from seconds to well under one on a fresh app — measured 2.7–5.3 s → 0.4 s on a fourteen-chapter tutorial app, with the same diagnostics.
...
- 68826a1: Split the over-long comment blocks in the default, api-only, blog, and SQLite templates so a freshly scaffolded app's first `bun run lint` reports no `guren/comment-length` warnings on framework-generated files.
...
- 21468a8: Declare the blog template's Post allowlist as the typed `defineModel` option
...
- 05dfef2: The scaffolded CI workflow is one `bunx guren gate --deps` step
...
- Ship template dependency ranges for this release
  ...
  The scaffold's `@guren/*` ranges are generated from the workspace versions,
  and this release moves `@guren/cli`, `@guren/orm`, `@guren/inertia-client`, `@guren/core`.
  `changeset publish` only uploads packages whose own version moved, so
  without this bump the updated ranges would sit in the repo and never reach
  anyone running `create-guren-app`.
...
- Ship template dependency ranges for this release
  ...
  The scaffold's `@guren/*` ranges are generated from the workspace versions,
  and this release moves `@guren/cli`, `@guren/core`, `@guren/testing`.
  `changeset publish` only uploads packages whose own version moved, so
  without this bump the updated ranges would sit in the repo and never reach
  anyone running `create-guren-app`.
...
- 3a72642: Restore the `guren make:module` note in the scaffolded `drizzle.config.ts`.
...
- 9144098: Widen both starter templates' TypeScript `include` so they cover the files they
  always meant to.
...
- Ship template dependency ranges for this release
  ...
  The scaffold's `@guren/*` ranges are generated from the workspace versions,
  and this release moves `@guren/cli`, `@guren/testing`.
  `changeset publish` only uploads packages whose own version moved, so
  without this bump the updated ranges would sit in the repo and never reach
  anyone running `create-guren-app`.
...
- 5042aa2: Refuse a scheme in DATABASE_URL for scaffolded SQLite apps
  ...
  | `file:local.db`   | `local.db`      | a file _named_ `file:local.db` |
  | `file::memory:`   | in-memory       | a file _named_ `file::memory:` |
...
- Ship template dependency ranges for this release
  ...
  The scaffold's `@guren/*` ranges are generated from the workspace versions,
  so a release that moves any of them has to republish the scaffolder — a
  `changeset publish` only uploads packages whose own version moved, and
  without this bump the updated ranges would sit in the repo and never reach
  anyone running `create-guren-app`.
...
- 72bd945: Exclude the health route that the templates actually define
