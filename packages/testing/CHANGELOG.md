...
- 05dbba3: Admit vitest 5 in the optional `vitest` peer range. `@guren/testing/vitest` runs unchanged on 5.0: the hooks it wires (`beforeEach`/`afterEach`, `vi.doMock`) kept their contract, and the suites of the reference apps pass on it.
...
- 8186771: `createGurenControllerModule()` now covers server-rendered content pages
...
- 545522b: Support multipart uploads in the controller test mock: `createControllerModuleMock()`'s `Controller` now implements `file()` and `files()`, and body parsing clones the request so `validateBody()` and `file()` compose on one multipart request — mirroring Hono's parse cache in the real runtime.
