...
One file survives `git clean` on purpose: `.env` is ignored, and `add auth` appended two blocks to its end: one that starts with a comment about the mail transport and sets `MAIL_MAILER` and the `SMTP_*` keys, and one that starts with a comment about `config/session.ts` and sets `SESSION_DRIVER`. Delete both. `db:status` should then list every migration as applied and none as orphaned. If one is orphaned, the server reloaded before you stopped it; `bun run db:reset` rebuilds the database from the migrations you have and drops every row with it. Start `bun run dev` again.
