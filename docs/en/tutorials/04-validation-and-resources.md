...
This is the biggest slice so far, which makes it the right place for this chapter's harness lever: the **`code-review` subagent** in `.claude/agents/code-review.md`. A subagent is an agent with its own brief and its own context, invoked by the main agent; this one's brief is a Guren code review. It runs `guren check` and `guren audit` first, then reads for what neither of them can see: whether the validation on every mutating route is the right validation, whether a resource sits in front of every record, whether the routes resolve in the order they are registered, and whether the tests cover the paths that fail. When your agent reports done, before you run the rubric yourself, ask:
