...
That route has been in `routes/web.ts` since chapter 1, and the `fly.toml` you are about to generate points its health check at it. It is also the one path `config/http.ts` excludes from host authorization, which matters more than it looks: in production the app answers only to the host `APP_URL` names, and a load balancer probing by IP is not that host. Health checks have to reach a server that is refusing everyone else.
