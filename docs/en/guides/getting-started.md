...
![The local Docs Graph viewer: document, spec, and code nodes joined by edges, with concept and relation counts and node-kind filters in the top-left](../../images/docs-graph.png)
...
- `CACHE_STORE`, `QUEUE_CONNECTION`, `MAIL_MAILER`: added by `guren add cache` / `guren add queue` / `guren add mail`, and read by the `config/cache.ts`, `config/queue.ts` and `config/mail.ts` they write. Each value has to name a store that file declares: an unknown `QUEUE_CONNECTION` or `MAIL_MAILER` fails the boot, and an unknown `CACHE_STORE` fails on the first cache call. `SESSION_DRIVER` appears once `guren add auth` (or `guren add session`) writes `config/session.ts`, which reads it; until then sessions live in process memory.
