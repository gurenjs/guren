...
- `CACHE_STORE`, `QUEUE_CONNECTION`, `MAIL_MAILER`: added by `guren add cache` / `guren add queue` / `guren add mail`, and read by the `config/cache.ts`, `config/queue.ts` and `config/mail.ts` they write. Each value has to name a store that file declares: an unknown `CACHE_STORE`, `QUEUE_CONNECTION` or `MAIL_MAILER` fails the boot. `SESSION_DRIVER` appears once `guren add auth` (or `guren add session`) writes `config/session.ts`, which reads it; until then sessions live in process memory.
