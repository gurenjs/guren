...
- The startup banner only renders in non-production environments by default; a production start prints one `[guren] Listening on http://<host>:<port>` line with the bound address instead. If you want to show it (or disable it explicitly) set `GUREN_DEV_BANNER=1` or `GUREN_DEV_BANNER=0`.
...
Before writing any output, `vercel:build` runs the deploy-runtime checks `guren doctor` reports and warns, without failing, on an in-memory session or OAuth store, a [Bun-only password hasher](/docs/guides/authentication#password-hasher), or filesystem provider discovery — each works locally and breaks across function invocations.
