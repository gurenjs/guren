...
- `ready()` imports `src/app.ts` and boots it once with `TestApp.fromApp()`. The file's `beforeAll` calls it, so the ORM is configured before any `beforeEach` you add to create or clear rows. A boot that fails is kept rather than thrown there, and each test that calls `ready()` or `client()` then fails by name with it. Open your own `beforeEach` with `await ready()`: otherwise a failed boot surfaces as your hook's database error, and `plan:verify` reads the run as red instead of `blocked`.
