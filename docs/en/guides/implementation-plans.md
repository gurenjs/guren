...
A validator's `fields` are read off the exported zod schema, a resource's from the payload type `guren codegen` reads, and a policy's abilities from its member names. A missing key or ability, or a field whose type, `required` or rule differs from the plan, reads `differ` and makes the element `drifted`, so `plan:verify` reports its step `incomplete`. Whatever the reader cannot be sure of, such as a field behind a transform, a refinement or a union, reads `unknown` rather than a guess, and `--json` gives the reason on the property. An element left with only `unknown` properties and existence matches needs a behaviour that reaches it, or a waiver, as above. The rules node by node are in the field-reader amendment to §6 of [RFC 0030](https://github.com/gurenjs/guren/blob/main/rfcs/0030-implementation-plans.md).
