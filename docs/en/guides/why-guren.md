...
**And it is measured, not aspirational.** A [reproducible agent evaluation](https://github.com/gurenjs/framework-comparison/tree/main/agent-eval) runs the same feature task, scored blind by typecheck, tests, and a hidden HTTP smoke the agent never saw. Across six rounds, 43 of 45 trials shipped a working feature. The sharpest round ran the day the v2.0.0 majors were published, when the model had no training data for the new APIs: with the scaffold's shipped harness, agents went 3/3 at a 29% lower median cost, while the stripped-guidance baseline passed only 1/3. Every round is published, including the ones that did not help, and the findings feed straight back into the framework.
