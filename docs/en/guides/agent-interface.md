...
**Plugin CSRF exemptions** — the one place an exemption made from inside
`node_modules` becomes visible. It names packages rather than paths: each path
is an argument computed at boot from that package's own configuration. A
declaration from a package outside the `@guren/` scope is a warning, since only
this repository can vouch for its own.
