...
- **Private attachments work on the binding alone via the signed delivery route.** Configure `delivery: {}` in `configureAttachments()` and mount `registerAttachmentRoutes(router)` in your route registrar: private attachments get path-relative signed URLs, and the route streams `get().body` through the Worker — no `presign` credentials needed. With `presign` configured, the driver declares `capabilities.presignedGet` and the same route upgrades to a 302 redirect to a short-lived presigned URL (carrying `Content-Disposition`/`Content-Type` as signed response overrides), so the bucket serves the bytes instead of the Worker. Public attachments are unchanged: they serve from the bucket's custom domain with zero Worker CPU.
