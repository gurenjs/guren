...
- **`runOnOneServer()` needs a `SchedulerLock` the isolates share.** `MemorySchedulerLock` lives in one isolate and so guards nothing here, and the scheduler refuses the task without a `lock`. Workers has no Redis, and KV has no conditional write, so implement `SchedulerLock` over a D1 insert on a unique key, or a Durable Object.
