...
Two placeholder styles are supported. Both insert values literally, and replacement values are never interpreted as patterns:
...
`guren check` validates translation catalogs (and `--i18n` runs just these checks, exiting non-zero on failures, for use in CI):
...
`lang/` is read from the filesystem, which deploy targets do not ship. The deploy builds (`guren cloudflare:build`, `guren lambda:build` and the Vercel build) read `lang/<locale>/*.json` at build time and bake the catalogs into their output, so an app using the default directory translates after deploy with no change. A file that is not valid JSON is left out, and the build warns. Changing a translation takes a rebuild and a new deploy.

The build reads `lang/` only. An app that sets `path`, or runs somewhere without a Guren deploy build (see the [Serverless guide](./serverless.md)), bundles the catalogs itself with a `MemoryLoader`, which wins over anything a build baked in:
...
`t()` resolves the manager from the default application, which is enough for a simple script. Avoid it in request handlers: a shared manager's `setLocale()` is process-wide state, so concurrent requests would override each other's locale. In apps wired through `createApp({ i18n })`, the request-scoped helpers (`this.t()`, `useTranslation()`) are safe: they resolve through the request and the app's container, never through shared mutable locale state.
