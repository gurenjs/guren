...
| `agent-route-tool-name:*` | The tool name (`agent.toolName`, or the route name) falls outside the MCP grammar `^[A-Za-z0-9._-]{1,128}# CLI Reference
...
Routes wrapped in named middleware (for example `router.middleware('auth').group(...)`) are recognized as protected. . A client rejects the whole tool list, not just the one tool. |
