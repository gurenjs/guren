...
lists each of them and changes nothing, `db/schema.ts` and `routes/web.ts`
included. `--force` overwrites every file on that list, your own among them. A file
that only `--test`, `--factory` or `--policy` adds is marked with its flag, and
dropping the flag gets past it. Promoting a prototype keeps the pages and validator
the prototype run wrote, so those never count.
