...
Lambda is not suited for serving static files. `lambda:build` stages `public/` into `.lambda/assets`, ready for S3, and the CDK construct (below) provisions the bucket and a CloudFront distribution that routes `/public/*`, plus each file or directory at the root of `public/` such as `/robots.txt`, to it, with the app as the default origin.
