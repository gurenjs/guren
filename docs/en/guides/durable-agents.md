...
It writes three things, and the two beyond the class are the point — an agent class alone is inert, because nothing loads it, nothing bounds it, and the deploy build cannot find it:
...
Everything beyond the new class is patched through the AST, and any patch the command cannot make is reported with the text to paste rather than skipped — an app whose agent looks registered and is not would be worse than a message.

Two things it does **not** do, both worth knowing before your first typecheck:

- **It does not touch `src/app.ts`.** Add `agentsPlugin(agents)` to `createApp({ providers })` yourself, as above.
- **The scaffolded class names an `Env` type and needs Workers types.** `GurenAgent<Env, TriagerState>` refers to an `Env` no fresh app defines, and the base class needs `@cloudflare/workers-types` in your tsconfig. Supply both:
...
with `bun add -d @cloudflare/workers-types`. Hand-written rather than generated, because the type is read by `tsc` and by your Bun test run, and neither can depend on a wrangler invocation having happened first.

Then import the type into the class the command wrote — the scaffold names `Env` but does not import it, and `@cloudflare/workers-types` declares `Cloudflare.Env`, never a bare `Env`:
...
One gap to know about on an API-only app: `guren check` and `guren audit` probe for `routes/api.ts`, but `guren tool:list` still defaults to `routes/web.ts` and needs the flag:
