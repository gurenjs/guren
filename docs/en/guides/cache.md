...
- Calls with different TTLs do not share a callback, and `rememberForever` counts as a TTL of its own, so every stored entry has the TTL its caller asked for.
