...
View components live in `app/View/*.tsx` (module-local: `modules/<name>/app/View/`), never under `resources/js/pages/`, since codegen claims that directory for Inertia pages. Every View file starts with a JSX pragma pointing at `@guren/core` and imports its types from there. Your app never declares `hono` as a dependency; the JSX runtime is re-exported through `@guren/core`, which the app already has:
...
Text children and attribute values are HTML-escaped automatically, so `{post.title}` is safe whatever the title contains. The `bodyHtml` injection is safe here because it comes out of a sanitizing renderer; see [the security boundary](#the-security-boundary) for where that responsibility sits.
...
`<script type="application/ld+json">` and `<style>` are *not* hoisted, so they render where you put them.
...
A `view()` response is plain HTML, so assert on the document text:
