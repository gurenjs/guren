...
Guren のレート制限は、アプリケーションを乱用から守るための仕組みです。複数のストレージバックエンド、カスタムキージェネレーター、固定ウィンドウとスライディングウィンドウの両方のアルゴリズムに対応しています。

## コアコンセプト

- **RateLimitStore**: レート制限データを保存するインターフェース。
- **MemoryRateLimitStore**: シングルプロセスのアプリケーション用のインメモリストア。
- **SlidingWindowRateLimitStore**: より正確なスライディングウィンドウの実装。
- **レート制限ヘッダー**: クライアントに現在の状況を伝える標準ヘッダー。
...
アプリが常にリバースプロキシや CDN(Cloudflare、ALB、Nginx)の背後にあるなら、`trustProxy` を有効にするだけでクライアント単位の制限ができます。独自の `keyGenerator` は要りません:
...
> **警告:** `trustProxy` は、すべてのリクエストが必ずプロキシを経由する場合だけ有効にしてください。直接公開しているデプロイでは、クライアントがヘッダーを偽装してクライアント単位の制限を回避できてしまいます。デフォルトが `false` なのはそのためです。
...
`headers: true`（デフォルト）なら、次のヘッダーがすべてのレスポンスに追加されます：
...
- **固定ウィンドウ**: ウィンドウの境界でカウンターをリセット。境界付近ではバーストを許してしまう。
- **スライディングウィンドウ**: リクエストごとのタイムスタンプを追跡。制限のかかり方がなめらか。
...
> [!NOTE]
> 独自のふるまいが必要な場合は、`@guren/core` の `RateLimitStore` インターフェース（`get` / `increment` / `reset`）を実装すれば、どんなストアでも使えます。
...
3. **可能な場合はユーザーでキー設定**: 認証済みユーザーをキーにすれば、1人のユーザーが他のユーザーに影響するのを防げる。
