...
**これは願望ではなく、計測済みの事実です。** 同一の機能追加タスクを、typecheck・テスト・エージェントには見えない HTTP スモークで盲検採点する[再現可能なエージェント評価](https://github.com/gurenjs/framework-comparison/tree/main/agent-eval)で、6 ラウンド計 45 トライアル中 43 が動く機能を出荷しました。最も示唆的なのは v2.0.0 メジャー公開当日のラウンドです。モデルの訓練データに存在しない新 API を相手に、scaffold 同梱のハーネスありでは 3/3 合格・コスト中央値 29% 減、ガイダンスを剥がしたベースラインは 1/3 しか合格しませんでした。効果がなかったラウンドも含めて全ラウンドが公開され、知見はフレームワーク本体へ還元されています。
