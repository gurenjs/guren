...
**これは願望ではなく、計測済みの事実です。** 同一の機能追加タスクを、typecheck・テスト・エージェントには見えない HTTP スモークで盲検採点する[再現可能なエージェント評価](https://github.com/gurenjs/framework-comparison/tree/main/agent-eval)で、エージェントは 5 ラウンド計 39 トライアルすべてで Guren 上に動く機能を出荷しました。同じ調査は、同梱のエージェントガイダンスがコストをどう変えるかをラウンドごとに — 効果がなかったラウンドも含めて — 計測・公開し、その知見はフレームワーク本体へ還元されています。ハーネス・生データ・制約事項はすべて公開されています。
