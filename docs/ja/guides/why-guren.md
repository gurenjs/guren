...
さらに Guren は、多くのフレームワークが投資してこなかった領域に力を入れています。**AI コーディングエージェント**がプロジェクトを理解し、拡張し、検証しやすくすることです。詳細は後述します。

## AI コーディングエージェントのために

多くのフレームワークは「ドキュメントを読む人間」に向けて設計されてきました。Guren はそれに加えて、「リポジトリの中で作業するエージェント」に向けて設計されています。そして新規アプリはすべて、最初からその状態で生成されます。secure by default と同じ精神の、agent-ready by default です。違いを生むのは次の 3 つの性質です。

**プロジェクトの把握が安上がり。** `bunx guren context` は、ルート・モデル・コントローラー・ページのコンパクトな地図を出力します(`--json` で機械可読)。エージェントはディレクトリツリーを這い回らずに、コマンド 1 つで正確なプロジェクト理解を得られます。コンテキストウィンドウは小さいまま、前提は正確なままです。データモデルについては `bunx guren model:list` が同じ役割を果たします。
...
ページ props、ルートパラメータ、リクエストボディはすべて型付きの契約です。エージェントが props 名をハルシネーションしたり、バリデーションスキーマを忘れたりすれば、diff を 1 行読むより前に、コンパイルエラーか check の失敗として現れます。
...
**そして仕様は自らを正直に保ちます。** Guren はプロジェクトの知識をひとつの原則で扱います: *導出できるものは導出し、できないものは宣言し、常に検証する(derived where possible, declared where not, checked always)*。ER 図・ドメインモデル・画面一覧はコードから生成され(`bunx guren spec:generate`)、CI のドリフトゲートで守られます。コードで表現できない意思決定は ADR として残り、frontmatter が統べるモデルへリンクし、変更のたびに検証されます。そして `bunx guren context Invoice` は、そのエンティティについてプロジェクトが知るすべてを（紐付いたドキュメントも含めて）、「リンクは切れておらず、導出ビューは最新だとチェッカーが検証した状態」で人間にもエージェントにも手渡します。全体像は[スペックアンカード開発](./spec-anchored.md)を参照してください。

**これは計測済みの事実です。** [Agents on Guren](https://github.com/gurenjs/agents-on-guren) は、バグ修正・セキュリティ・機能追加の 20 タスクを、エージェントには見えない受け入れテストで採点する評価です。3 モデルそれぞれで、scaffold 同梱のハーネスあり・なしの両方を走らせた計 360 ランです。各ランの patch、ログ、判定はすべて公開しています。Sonnet 5 ではハーネスありが 60/60 ラン合格、なしが 58/60 で、ターン数 28% 減・コスト 25% 減でした。Opus 5 はどちらも 60/60 合格で、差はターン数 26% 減・コスト 12% 減に出ました。Haiku 4.5 は合格率そのものが動き、51/60 から 54/60 へ上がりました。ただしコストは 6% 増えています。`guren check` を実際に実行したランは、ハーネスありで 180 中 119、なしでは 15 でした。ハーネスが逆効果になったケースもレポートに載せています。この評価は `@guren/orm` の `paginate()` が eager loading を落とすバグも掘り当て、すでに起票済みです。

## Hono との比較

Guren の HTTP レイヤーは Hono *そのもの*です。すべてのリクエストが Hono のルーターを通るため、パフォーマンスクラスは Hono と同じです。Guren が足すのは薄い MVC レイヤーで、新しいサーバーではありません。
...
数個の JSON エンドポイントで済むサービスなら、素の Hono は優れた選択です。そして Guren の中身は Hono なので、片方からもう片方へ育てるのは書き直しではなく自然な成長になります。差が出るのはアプリが大きくなってからです。上の表の各行は、Guren がなければ自分で書き、テストし、保守し続けるグルーコードです。

## Next.js との比較

Next.js と Guren は答えている問いが違います。Next.js は React ファーストで、レンダリング戦略(RSC、ストリーミング、ISR)が中核の抽象です。バックエンドはフロントエンドから育っていきます。Guren はサーバーファーストで、Laravel の系譜に連なる MVC アプリケーションです。React は Inertia を通じて届けられるビューレイヤーになります。
...
Laravel や Rails の経験者なら、Guren はすぐに馴染みます。コントローラー、モデル、ミドルウェア、ポリシー、リソース、ジョブ、`make:*` ジェネレーター。どれも期待どおりに振る舞います。変わるのは言語の境界です。というより、境界がなくなります。
...
- **極小フットプリントのサービスが欲しい。** Webhook レシーバーやエッジファンクションに MVC は不要で、素の Hono の方が軽量です。
...
- [First Steps](./first-steps.md): 1 つのリクエストが全レイヤーを通る流れを 10 分で追います。
