...
`scheduler.start()` はすぐに一度 tick を実行し、その後はアプリサーバーなどが動かし続けるプロセスの中でだけ tick を続けます。`scheduler.start()` を呼ぶだけの単体スクリプトは、その時点で実行時刻を迎えたタスクを動かしたあと、次の tick を待たずに終了します。実行時刻を迎えたタスクをプロセスの外から実行するときは、cron から `bunx guren schedule:run` を呼び出してください。このコマンドが読むのはスケジュールカーネルに登録したタスクです。詳しくは[CLIから見えるようにする](#cliから見えるようにする)を参照してください。
