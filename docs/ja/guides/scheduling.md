...
スケジューラーは、アプリサーバーなどがすでに動かし続けているプロセスの中で時刻を刻みます。`scheduler.start()` を呼ぶだけの単体スクリプトは、次の tick を待たずに終了します。プロセスの外から実行するときは、cron から `bunx guren schedule:run` を呼び出してください。詳しくは[CLI統合](#cli統合)を参照してください。
