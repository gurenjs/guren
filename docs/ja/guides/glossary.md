...
- **Hono**: 軽量な HTTP サーバーフレームワーク。Guren は Hono の上にルーティングやコントローラを載せています。
...
- **Route（ルート）**: パスと HTTP メソッドを、処理（コントローラや関数）に結びつける定義。
- **Controller（コントローラ）**: リクエストを受け取り、モデルでデータを扱い、レスポンスを返すクラス。
