...
ローカルディスクでは、シンボリックリンクでディスクの外へ出るパスを拒否します。リンク先が存在せず、その位置がディスクの外にある場合も同じです。ディスク内で完結するリンクは通常どおりたどります。設定した `root` 自体もシンボリックリンクで構いません。処理中に別プロセスがディレクトリを差し替える競合は防げないため、ルートと親ディレクトリの書き込み権限は信頼できるユーザーに限定してください。
...
5. **署名付きURLを使用**: プライベートファイルには公開せず一時URLを生成。モデルアタッチメントでは[署名配信ルート](./attachments.md#url-と可視性)がどのドライバでもこれをカバーします(local ディスクやバインディングのみの R2 も含む)。
