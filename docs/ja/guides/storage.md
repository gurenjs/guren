...
ローカルディスクでは、`root` 配下のシンボリックリンクを拒否します。リンク先が存在しない場合も同じです。設定した `root` 自体はシンボリックリンクでも構いません。処理中に別プロセスがディレクトリを差し替える競合は防げないため、ルートと親ディレクトリの書き込み権限は信頼できるユーザーに限定してください。
