...
コンソールコマンドを使うと、バックフィル・単発のメンテナンス・レポート生成といった処理を、HTTP ハンドラと同じモデル・サービス・コンテナのままターミナルから実行できます。
...
必須の引数が省略された場合、`argument()` は `undefined` を返します。パーサは弾いてくれないので、必須にしたい値は自分で検証してください。真偽値オプションは常に定義済みです（フラグがなければ `false`）。
