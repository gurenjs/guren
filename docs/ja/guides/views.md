...
`this.view()` は JSX コンポーネントをプレーンなサーバーレンダリング HTML レスポンスとして返します。`this.inertia()` と対になる、ハイドレーションしない方の仕組みで、ブログ記事、ドキュメント、マーケティングページのような公開・読み取り中心のページに向いています。クライアントフレームワークなし、ハイドレーションなし、Inertia のページペイロードスクリプトもドキュメントに含まれません。guren.dev 自身のブログ記事がこの仕組みで配信されています。
...
実際のブログの公開ページを Inertia から移行したときの実測では、同じ記事が Inertia SSR ドキュメントとして 443 KB、プレーンなサーバーレンダリング HTML として 144 KB でした。guren.dev では、この重複したペイロードが圧縮してもかなり残り、docs ページの gzip 後レスポンスの 33.7% を占めていました。
...
| React に状態を持つ画面 | ハイドレーションするほどの動きがないページ |

両者は 1 つのアプリで共存します。guren.dev のブログは記事ページを `view()` で配信しつつ、管理画面のエディタは Inertia のままです。

## 最初の View コンポーネント

View コンポーネントは `app/View/*.tsx`（モジュールローカルなら `modules/<name>/app/View/`）に置きます。`resources/js/pages/` 配下には置かないでください。このディレクトリは codegen が Inertia ページ用として使っています。View ファイルはいずれも `@guren/core` を指す JSX プラグマで始め、型も `@guren/core` からインポートします。アプリが `hono` を依存に追加することはありません。JSX ランタイムは、アプリがすでに持っている `@guren/core` を通じて再エクスポートされています。
...
**Layout 自身の `<head>` には、ページが決して上書きしないものだけを置きます**。charset、viewport、スタイルシートのリンク、RSS 発見リンクのようなサイト全体のタグです。ページごとのメタデータ（`<title>`、description、canonical URL）はページコンポーネントに属します。これは単なる整理整頓の話ではありません。ページがレンダリングしたメタデータは `<head>` に追記されるだけで、既存のタグを置き換えません。重複排除は Layout のリテラルな子要素を対象にせず、ブラウザは最初に見つけた `<title>` を使います。そのため、Layout にハードコードしたデフォルトの `<title>` は、すべてのページのタイトルを黙って覆い隠してしまいます。

**ページのメタデータは body ではなく、Layout の `head` スロットで渡します**。ツリーのどこでレンダリングされた `<title>`、`<meta>`、`<link>` もネイティブに `<head>` へ巻き上げ（hoisting）られるので、深くネストしたコンポーネントからでもメタデータを供給できます。ただし hoisting は巻き上げるタグ 1 つごとにドキュメント全体を再走査するため、タグ数に対して二乗のコストがかかります。body に置いた 15 タグの SEO ブロックはレンダリングごとに約 1 ミリ秒を消費し、ページサイズとともに増えます。上の Layout の `head` スロットなら、同じタグを一定のコストで `<head>` に直接レンダリングできます。hoisting はツリーの深いところで出力されるタグのための安全網で、速いのはスロットの方です。

`<script type="application/ld+json">` と `<style>` は巻き上げられません。書いた場所にそのままレンダリングされます。

## アセット解決: `viteAsset()`

コンテンツページにはスタイルシートの URL が必要ですが、その URL は環境によって変わります。`viteAsset(entry)` がこの 2 通りを引き受けます。
...
サーバーレスのバンドルはビルド出力ディレクトリを伴わずに配備されるため、実行時に読めるマニフェストファイルがありません。デプロイプラグイン（`@guren/plugin-cloudflare`、`@guren/plugin-vercel`、`@guren/plugin-lambda`）はビルドステップでマニフェスト JSON を環境変数 `GUREN_VITE_MANIFEST` に注入し、`viteAsset()` はファイルシステムよりそちらを優先します。アプリ側の設定は要りません。ランタイムが `public/assets/manifest.json` をまったく見ないターゲットでも、`view()` ページは動きます。

## 完全なドキュメントとフラグメント

ページを Layout でラップし忘れても、その失敗は表に出ません。ページは 200 を返しますが、巻き上げ先の `<head>` がないため、すべての `<title>` と `<meta>` は body 内にインラインのまま残り、スタイルシートもリンクされません。開発中ならスタイルの欠落に気づけますが、クローラーが body から SEO タグを読んでいることに気づくのはずっと後になります。

そのため `view()` は、この場合はっきりと失敗させます。`<html>` をルートとするドキュメントではなくフラグメントをレンダリングしたコンポーネントは、最初のレンダリングで理由を説明するエラーを投げます。フラグメントを意図している場合（たとえばウィジェット用の HTML 部品）は `{ doctype: false }` を渡してください。
...
自動エスケープはマークアップと属性の突破を防ぎます。テキストの子要素にある `<script>` タグはテキストとしてレンダリングされ、属性値の中の `"` が属性を打ち切ることもありません。残りの 2 つは自分で守る必要があります。
...
- **HMR はありません**。`view()` ページは Vite クライアントを載せないため、ホットリロードの対象がそもそもありません。編集したらリロードしてください。そのかわり、自分で足さない限りクライアント JavaScript はゼロです。
...
Vitest でコントローラーを単体テストする場合、`@guren/testing` の `createControllerModuleMock()` が `view()` に対応しており、`viteAsset()` のモックもエクスポートしています（`@guren/testing` 1.7.0 以降が必要です）。どちらも実際のレンダリングエンジンに委譲するため、エスケープ、フラグメントガード、アセット解決は本番とまったく同じ挙動になります。テスト中の `viteAsset()` は決まった開発サーバー URL を返します。コントローラーテストのヘルパー全般は[テストガイド](./testing.md)を参照してください。
