...
シグナルを無視する処理はタイムアウト後も実行を続けます。その処理が正常に
終了した場合、ジョブは再試行されず完了扱いになります。
JavaScript では任意のプロセス内処理を強制停止できないため、停止しない
ワーカーはプロセス監視で終了させてください。`stop()` は停止を要求し、
ワーカーのタイムアウト時間まで待機します。`start()` は実行中の処理が
終了するまで完了しません。

Redis の予約はキャンセル中も更新されます。更新がドライバーのエラーで失敗した
場合は次のハートビートで再試行します。予約が別のワーカーへ渡っていた場合は
シグナルを中断し、処理の終了後は完了通知も返却も行わず `jobFailed` で報告して
次のジョブへ進みます。ジョブは予約期限の
...
`SqsDriver` は `ChangeMessageVisibility` で予約を更新します。キュー属性と同じ
`visibilityTimeout`(秒)をオプションで渡してください。期限付き予約を使う独自
ドライバーでは `heartbeatInterval` と `extendReservation(job)` を実装するか、キャンセル処理を含む実行時間より
