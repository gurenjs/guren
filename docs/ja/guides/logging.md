...
Guren のロギングは、RFC 5424 に準拠したログレベル、複数のチャンネル、コンテキスト付きの記録に対応しています。
...
いちばん手軽なのは `LogManager` 経由での記録です。コンテナから遅延解決されます。
...
Guren は RFC 5424 の重大度レベルを扱えます（重大なものから順に並べています）。
...
ログファイルを毎日ローテーションし、古いものは自動で削除します。
...
ファイル名には日付が付きます（`app-2024-01-15.log`）。
...
ログを書く側はすべて、自分が動いているコンテナから `log` を解決します。そのためコントローラ・ジョブ・コマンドには登録なしで届きます。

> [!NOTE]
> `setLogManager()` と `getLogManager()` は 2.23.0 で非推奨になり、3.0.0 で削除されます。プロセスにつき 1 つのマネージャを読み書きするため、1 プロセスに 2 つのアプリがあると共有されてしまいます。`bunx guren upgrade` が書き換えます。
...
> [!NOTE]
> Cloudflare Workers では、レスポンスを返した時点で終わっていない処理は破棄されます。リクエストの処理中に `log()` を呼ぶと、非同期チャンネルの書き込み（上の例の `fetch`）はそのリクエストの `waitUntil` に渡されます。そのためレスポンスの後でもエントリが届きます。起動時などリクエストの外で呼んだログは、`waitUntil` を通さずに書き込まれます。ほかのランタイムではプロセスがリクエストより長く動き続けるので、影響はありません。
...
1. **レベルを使い分ける**: 何でも error にしない。開発中の情報は debug に回します。

2. **コンテキストを添える**: ユーザー ID やリクエスト ID など、デバッグに要る情報を一緒に記録します。

3. **構造化して記録する**: 本番では JSON フォーマットのほうが解析も検索も楽です。

4. **ログファイルをローテーションする**: ディスクを埋めないよう daily ドライバーを使います。

5. **本番はレベルで絞る**: 本番のコンソールは 'info' 以上に設定します。

6. **コンテキスト付きロガーを作る**: リクエスト単位の記録には `withContext()` を使います。

7. **機密データを記録しない**: パスワード、トークン、個人情報はログに出しません。

8. **終了時にチャンネルを閉じる**: 書き込み待ちを流し切るため `log.close()` を呼びます。
