...
Guren のロギングは、RFC 5424 に沿ったログレベル、複数のチャンネル、コンテキスト付きの記録に対応しています。

## コアコンセプト

- **LogManager**: ログのチャンネルをまとめて管理する中心のオブジェクト
- **Logger**: コンテキストを付けてログエントリを書き込むインスタンス
- **LogChannel**: ログエントリの出力先（コンソール、ファイルなど）
- **LogLevel**: RFC 5424 に沿った重大度（emergency、alert、critical、error、warning、notice、info、debug）

## 基本的な使い方

### ファサードを使用（推奨）

いちばん手軽なのは、`LogManager` を通して記録する方法です。`LogManager` は、使うときにコンテナから解決されます。
...
`LogManager` を自分でインスタンス化して使うこともできます。
...
ログレベルには RFC 5424 の重大度を使います。次の例は重大なものから順に並べています。
...
ログファイルを日ごとに切り替え、古いファイルは自動で削除します。
...
ファイル名には `app-2024-01-15.log` のように日付が付きます。

### スタックチャンネル

複数のチャンネルをまとめて 1 つのチャンネルとして扱えます。
...
`LogServiceProvider` は、`createApp({ logging })` の設定から組み立てたマネージャをコンテナに束縛します。マネージャを自分で組み立てる場合は、独自のプロバイダから束縛してください。
...
> [!NOTE]
> `setLogManager()` と `getLogManager()` は 2.23.0 で非推奨になり、3.0.0 で削除されます。この 2 つはプロセスに 1 つだけのマネージャを読み書きするので、1 つのプロセスに 2 つのアプリがあるとマネージャが共有されてしまいます。`bunx guren upgrade` が自動で書き換えるのは、プロバイダやジョブの中の呼び出しだけです。モジュールのトップレベルにある呼び出しは報告されるだけなので、上のような束縛に手で置き換えてください。
...
> [!NOTE]
> Cloudflare Workers では、レスポンスを返した時点で終わっていない処理は破棄されます。リクエストの処理中に `log()` を呼んだ場合、非同期チャンネルの書き込み（上の例の `fetch`）はそのリクエストの `waitUntil` に渡されるので、レスポンスを返した後でもエントリは届きます。起動時のようにリクエストの外で呼んだログは、`waitUntil` を通さずに書き込まれます。ほかのランタイムではプロセスがリクエストより長く動き続けるので、この点は気にしなくて構いません。
...
1. **レベルを使い分ける**: 何でも error にはせず、開発中にだけ見たい情報は debug にします。

2. **コンテキストを添える**: ユーザー ID やリクエスト ID など、デバッグに必要な情報を一緒に記録します。

3. **構造化して記録する**: 本番では JSON 形式にしておくと、解析も検索も楽になります。

4. **ログファイルをローテーションする**: ディスクがいっぱいにならないよう、daily ドライバーを使います。

5. **本番ではレベルを絞る**: 本番のコンソールは 'info' 以上だけを出すように設定します。

6. **コンテキスト付きのロガーを作る**: リクエスト単位で記録するときは `withContext()` を使います。

7. **機密データを記録しない**: パスワード、トークン、個人情報はログに出さないでください。

8. **終了時にチャンネルを閉じる**: 書き込み待ちのエントリを出し切るため、`log.close()` を呼びます。
