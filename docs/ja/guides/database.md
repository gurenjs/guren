...
SQLite は接続をひとつしか持たないため、同時に扱えるトランザクションはひとつだけです。同時に開始されたトランザクション、たとえば `Promise.all([Post.transaction(…), Post.transaction(…)])` は順番待ちのうえ逐次実行され、それぞれが個別にコミットまたはロールバックされます。すでに開いているトランザクションの内側から始まったトランザクションは順番待ちにできない（自分自身の完了を待つことになる）ため、エラーで拒否されます。`Model.transaction()` を入れ子にしないこと、そしてコールバックの中でデータベース以外の処理（HTTP リクエスト、タイマー、ファイル読み込みなど）を await しないことが必要です。PostgreSQL や MySQL のようなプール接続のデータベースは影響を受けません。
