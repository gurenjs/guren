...
コールバックの中からコミット済みの状態を読みたいときは、読み取りを `Model.outsideTransaction()` で包みます。その中の呼び出しは開いているトランザクションに合流せずプールへ流れるので、冪等性チェックやテストの前提確認で他の接続と同じ景色が見えます。SQLite ではプールがトランザクションと同じ接続なので、未コミットの書き込みもそのまま見えます。

トランザクションの内側で開かれた `Model.transaction()` は、新しいトランザクションを開かずに外側へ合流します。内側のコールバックには同じハンドルが渡され、その書き込みは外側と一緒にコミットまたはロールバックされます。セーブポイントは作られないため、内側のエラーを外側のコールバックが握りつぶすと、内側の書き込みは外側のトランザクションが確定するまで残ります。
