...
フォルダを消して間に合うのは、まだどの起動も適用していないときだけです。ジェネレーターがファイルを書き換えている最中に dev サーバーがリロードすると、そちらが先に適用します。そうなると、どのフォルダにも対応しない行がトラッカーに残ります。マイグレーターはその行を読み飛ばすので、そのマイグレーションが作ったものはデータベースに残ったままです。`db:status` はその行を orphaned と表示し、起動のたびにマイグレーション前の警告が出ます。
