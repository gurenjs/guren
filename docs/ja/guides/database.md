...
SQLite は接続をひとつしか持たないため、2つのトランザクションを同時に開くことはできません。別のトランザクションが開いている間に始まったトランザクションは、黙ってそこへ合流するのではなくエラーで拒否されます。`Model.transaction()` を入れ子にしないこと、そしてコールバックの中でデータベース以外の処理（HTTP リクエスト、タイマー、ファイル読み込みなど）をawait しないことが必要です。PostgreSQL や MySQL のようなプール接続のデータベースは影響を受けません。
