...
スキャフォールドしたアプリは、データベースに最初に接続するときに未適用のマイグレーションを適用します。パイプラインで実行していなくても、コンテナは起動時にマイグレーションします。それでも、新しいイメージを展開する前にパイプラインで実行しておくほうが安全です。マイグレーションが失敗した時点でデプロイを止められ、起動したコンテナは適用するものがない状態で立ち上がります。
...
例外は 2 つです。Data API のアダプタが起動時にマイグレーションするのは `migrateOnStart` を指定したときだけです。D1 のマイグレーションは `wrangler d1 migrations apply` で適用するので、起動時には走りません。詳しくは[マイグレーションが走るタイミング](./database.md#マイグレーションが走るタイミング)を参照してください。
