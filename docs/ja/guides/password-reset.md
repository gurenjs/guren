...
Guren のパスワードリセットは、トークンの生成・検証・有効期限をまとめて扱います。トークンは保存せずに署名し、ストアに渡すのは不透明なトークン ID だけです。
...
物理カラム名は自由に決められます。決まっているのはストアのメソッドシグネチャだけです。このガイドでも以前は、ストアがトークンのハッシュを保持していた頃の名前 `token_hash` を使っていました。すでにそのカラムがある場合は、移行せずそのまま `tokenId` に対応づけてください。
...
1. **パスワード忘れは常に成功を返す**: メール列挙攻撃を防ぐため、常に成功メッセージを表示する。

2. **短い有効期限を使用**: パスワードリセットトークンは短時間（15〜60分）で期限切れにする。

3. **パスワード変更時に無効化**: ユーザーがパスワードを変更したら、すべてのリセットトークンを削除する。

4. **リクエストをレート制限**: パスワード忘れエンドポイントをレート制限して乱用を防ぐ。

5. **HTTPSを使用**: リセットリンクには機密性の高いトークンが含まれるので、HTTPS経由で送る。

6. **メールにパスワードを含めない**: 送るのはリセットリンクだけにして、新しいパスワードは送らない。

7. **リセット試行をログ**: セキュリティ監査のため、パスワードリセットのリクエストをログに残す。

8. **パスワード変更をユーザーに通知**: パスワードの変更が完了したらメールを送る。
