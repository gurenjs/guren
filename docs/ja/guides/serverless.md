...
通常のワーカーで独自アダプターを使う場合、`deleteMessage({ queueUrl, receiptHandle })` を実装し、`receiveMessage()` から SQS の `ApproximateReceiveCount` に基づく正の整数 `receiveCount` を返してください。ジョブの送信だけに使うアダプターでは、どちらも省略できます。Lambda のイベントソース経由では、引き続き `createSqsHandler()` と AWS のバッチ処理結果を使います。
