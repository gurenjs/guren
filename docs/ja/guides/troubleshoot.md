...
2. 開発環境では、バリデーションエラーは 422 レスポンスとして返り、ボディの JSON に各フィールドのエラーが並びます。ブラウザのネットワークタブでレスポンスボディを確認してください。
...
- **Error**: 修正しないとアプリが正しく動かないもの(例: ルートが存在しないコントローラーを参照している)
- **Warning**: 動きはするが意図しない挙動になりうるもの(例: ページコンポーネントがあるのにどのルートからも参照されていない)
