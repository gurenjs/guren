...
同じプロセス内で同じキーへの呼び出しが同時にミスすると、コールバックは1回だけ実行されます。呼び出し元は全員が同じ結果を受け取ります。成功なら同じオブジェクト、失敗なら同じ例外です。例外を投げたコールバックは何もキャッシュしないため、次の呼び出しで再実行されます。対象は `cache.store()` で取得したストアの `remember` と `rememberForever` です。タグ付きキャッシュと `registerStore()` で追加したストアも含みます。`new MemoryStore()` のように直接作ったストアはコールバックを共有しません。

共有の範囲はプロセス単位です（Cloudflare Workers では isolate 単位）。別々のサーバーが同時に同じキーでミスした場合は、それぞれがコールバックを実行します。

- 呼び出し元は同じオブジェクトを受け取るので、結果は読み取り専用として扱ってください。ある呼び出し元が加えた変更は、他の呼び出し元にも見えます。変更が必要なら先にコピーしてください（`structuredClone(posts)`）。メモリストアはヒットのたびに保存済みのオブジェクトそのものを返すため、変更はキャッシュ内の値にも及びます。
- 同時の呼び出しが異なる TTL を渡した場合は、最初の呼び出しの TTL で保存されます。
- 実行中のコールバックの開始から10秒以上たって届いた呼び出しは、待たずに自分のコールバックを実行します。すでに待っている呼び出し元は最初のコールバックを待ち続けます。コールバック内の遅い I/O には個別にタイムアウトを設定してください。
- 同じストア経由でそのキーに書き込むと（`set`、`delete`、`setMany`、`deleteMany`、`clear`、タグ付きキャッシュの `set` と `delete`）、以降の呼び出しは書き込み前に始まったコールバックに合流せず、新しく実行します。
...
カウンターの加算・減算は元の有効期限を維持します。ファイルストアはインスタンス・プロセス間で共有するファイルシステムロックとアトミックなファイル置換を使います。ロック待ちが5秒を超えると例外になります。残った `.lock` ディレクトリを削除する際は、書き込み元の停止を確認してください。
