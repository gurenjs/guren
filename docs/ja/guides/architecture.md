...
Bun 1.3.x では、broadcasting の WebSocket クライアントが接続したまま `bun --hot` でリロードするたびに、この上限まで待つことになります。リロードは古いサーバーを止める前にすべてのソケットを閉じます。これらのバージョンでは、サーバー側が WebSocket を閉じた後の `stop()` が完了しません。クライアントを1つ接続して計測すると、Bun 1.3.11 と 1.3.14 ではリロードごとに約5.1秒かかり、警告が出ました。Bun 1.4.2 では同じリロードが約100msで終わります。Bun 1.4.0 以降に上げてください。1.3.x のままで使う間は `GUREN_BUN_STOP_TIMEOUT_MS` を数百ミリ秒に下げてください。ソケットはすでに閉じているため、待つべき処理は残っていません。
