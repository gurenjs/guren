...
`bun --hot` のリロードだけは例外です。置き換える古いサーバーの待ち時間は250msで、超えても警告は出ません。この停止は強制クローズで、リロードは broadcasting のソケットをすべて閉じた後なので、待つべき処理は残っていません。この短い上限が効くのは Bun 1.3.x です。これらのバージョンでは、サーバー側が WebSocket を閉じた後の `stop()` が完了しません。broadcasting のクライアントを1つ接続して計測すると、Bun 1.3.11 と 1.3.14 では既定の上限でリロードごとに約5.1秒かかっていました。いまは約0.3秒です。Bun 1.4.2 では、どちらでも同じリロードが約100msで終わります。
