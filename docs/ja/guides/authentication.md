...
このモードではプロフィールページのメールアドレスは読み取り専用になり、`ProfileUpdateSchema` からもフィールドごと外れます。ここで変更されたアドレスを検証する仕組みが存在しないため、編集可能なままだと「一度も証明していないアドレスを名乗る」ことができてしまい、その後 `OAuthController` の重複チェックによって本来の持ち主が初回サインインを拒否されるからです。
