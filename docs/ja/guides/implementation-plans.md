...
- `ready()` は `src/app.ts` を import し、`TestApp.fromApp()` で一度だけ起動します。ファイルの `beforeAll` がこれを呼ぶので、行の作成や削除のために追加する `beforeEach` より先に ORM が設定されます。起動の失敗は `beforeAll` では投げずに保持し、`ready()` か `client()` を呼んだ各テストがその失敗で名前付きで失敗します。追加する `beforeEach` は `await ready()` から始めてください。そうしないと起動の失敗がフックのデータベースエラーとして現れ、`plan:verify` は `blocked` ではなく失敗した実行として読みます。
