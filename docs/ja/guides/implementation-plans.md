...
読める性質がすべて承認時にすでに一致していた `alter` は、性質では完了しません。`plan:approve` はそうした計画も承認し、要素と性質を挙げて警告します。`--json` では `heldAlters` に並びます。警告は承認の項目の読み取りから判定します。同じハッシュを承認し直すと同じ警告が出て、同じ baseline のもとなら、実装のあとに承認し直しても、実装で変えた性質については警告しません。次の例は、`view.posts.show` がページですでに宣言済みの `post` prop を書き直しただけの計画です。
