...
その結果、人間にとってもリポジトリで作業するすべてのエージェントにとっ
ても、コードが動いても嘘をつかない仕様が手に入ります。
...
をそのまま見せます。手で編集しないでください。信頼を人に頼らないための
ドリフトゲートがあります:
...
- `generated` と `verified` は「誰が書いたか」「誰が確認したか」をOKFの
  ...
  で記録します。エージェントが保守するコーパスでは、この来歴こそが文書の
  信頼を支えます。
...
到達できません)。バンドル全体をインタラクティブなリレーショングラフ(文書・
エンティティ・コードパスがノード、検証済みリンクがエッジ)として
描画し、ノードをクリックするとfrontmatter・信頼度(trust tier)・リンク検証結果
...
ガードが確認するのは接続元で、呼び出し元のアプリではありません。接続を
ローカルで終端して開発サーバーへ転送するもの(リバースプロキシ、コンテナ
のポート公開、ngrok のようなトンネル)はループバックの接続元として見える
ため、その背後のトラフィックは受け入れられます。`GUREN_MCP=1` で起動した開発
...
エージェントは嘘を全幅の信頼で読むからです。導出ビューはコードから再生成
