...
だけなので、ローカルで gate を通した変更は CI も通ります。Claude Code のハーネスは
...
| `evidence` | 意味 |
|------------|------|
| `manifest` | イントロスペクションの結果を使って判定した。マニフェストにない事実(OAuth の state ストア、キュー、明示的に生成したインメモリストア)はソースの走査で補う |
| `static` | ソースの走査で判定した。イントロスペクションが失敗したか省略された場合、判定がマニフェストの事実を使わない場合(provider の自動検出、ディスクの `root`)、判定に使う情報をアプリが保証できない場合のいずれか。最後のものは、`register()` で例外を投げた provider がある場合、未設定の環境変数を読む設定がバインドされなかった場合、deferred provider か自身を説明できないバインディングがそのセクションを提供する場合、ソースに `configureAttachments()` の呼び出しがあるのに登録中にエンジンが設定されなかった場合で、メッセージに理由が書かれる |
| `none` | 代わりに使える根拠がない事実(キャッシュストアがプロセス内か)。キーの末尾は `-unverified` で、結果は必ず警告になる |

イントロスペクションが失敗すると、`check` は理由を書いた advisory の
`introspection-unavailable` を 1 行加え、判定はソースの走査に戻ります。`doctor` は
理由を JSON の `evidenceReason` に入れます。`--no-introspect` で実行した場合は付きません。
`--no-introspect` を付けるとイントロスペクションを行わず、ソースだけで判定します。
エントリがまだ import できないアプリで使います。
...
- アプリのどこにも alias や group として登録されていない名前は、未解決として報告します。
  そのルートは boot 時のマウントで失敗するため、同じチェーンのガードより先に判定します。
  イントロスペクションが実行しないものがその名前を登録しうる場合(`createApp({ boot })` の
  コールバック、`register()` の代わりに `introspect()` フックを実行する provider)は、
  メッセージにそれを書き、ガードやアクション内の `userOrFail()` があればルートは pass します。
- コントローラはファイルと export で特定します。2 つのモジュールがそれぞれ `ReportController`
  を宣言していても、各ルートは自分のクラスで判定されます。コントローラのファイル以外
  (ルートファイルの中やパッケージ)で宣言したクラスは、同じ名前のクラスをコントローラの
  ファイルが宣言していても、そのボディでは判定しません。ルートは解析できないものとして
  報告します。`controller-name-collision:*` は、宣言するファイルの import が失敗した、
  またはクラスが再 export を通して見つかったためにマニフェストがルートのクラスを特定できず、
  2 つのファイルがその名前を宣言している場合に限って報告します。ボディの検査
  (`validateBody()`、`userOrFail()`)は引き続きアクションのソースを読みます。

`guren check` のエージェント公開ルートのルールも、2 つのファイルが宣言するクラス名を
エージェントルートが指している場合に、同じ方法でコントローラを特定します。生 SQL、
