...
編集のたびに codegen を実行し、`guren check`、`doctor`、`guren gate` は既定の codegen が書く
内容と比べるためです。`--introspect` を付けると、どのルートがどの順で存在するかはアプリが決め、
...
`guren codegen` がそれを削除します。アプリをイントロスペクションできない場合と、`--routes` が `check` の見つける
