...
呼ぶアプリ、`Attachable(...)` を mixin するモデルがあるアプリ、`.agent()` を宣言した
ルートがあるアプリが対象です。1 回の実行につき
...
- アプリのどこにも alias や group として登録されていない名前は、未解決として報告します。
  ...
  `introspect()` フックを実行した provider、`register()` が例外を投げた provider)は、
  メッセージにそれを書きます。
- コントローラはファイルと export で特定します。2 つのモジュールがそれぞれ `ReportController`
  を宣言していても、各ルートは自分のクラスで判定されます。ルートのクラスがコントローラの
  ファイルのどの export とも一致せず(ルートファイルの中で宣言したクラスなど)、同じ名前の
  クラスをいずれかのファイルが export している場合、そのボディは使わず、ルートは解析できない
  ものとして報告します。コントローラのファイルが export せずに宣言したクラスは、自分の
  ボディで判定します。`controller-name-collision:*` は、宣言するファイルの import が失敗した、
...
`guren check` のエージェント公開ルートのルールも同じ方法でコントローラを特定するので、
エージェントルートがあるアプリではイントロスペクションを行います。生 SQL、
