...
- 認可はするが認証をしないチェーンは警告のままで、メッセージに確認する ability の名前が
  入ります。ゲストのリクエストは `null` ユーザーのままゲートに届き、ポリシーが通すこともあります。
- アプリのどこにも alias や group として登録されていない名前は、同じチェーンにガードが
  あっても先に未解決として報告します。そのルートは boot 時のマウントで失敗します。
  `createApp({ boot })` のコールバックがその名前を登録する場合は別です。
- コントローラはファイルと export で特定します。2 つのモジュールがそれぞれ `ReportController`
  を宣言していても、各ルートは自分のクラスで判定されます。`controller-name-collision:*` は、
  クラス名だけでたどったルートがあるクラス(ルートファイルの中で宣言したクラスなど)に
  限って報告します。ボディの検査(`validateBody()`、`userOrFail()`)は引き続きアクションの
  ソースを読みます。
