...
デプロイプラグインか Lambda アダプタを宣言したアプリでは、`guren check` と
`guren doctor` がデプロイ実行環境の判定をまずイントロスペクションの結果から行います。
対象は auth manager が持つハッシャー、選ばれているセッションストアとキャッシュストア、
登録中に例外を投げた provider の有無です。イントロスペクションは 1 回の実行につき
最大 1 回で、デプロイ先が見つかったときだけ行います。マニフェストは実行環境の `.env` を
読み込んだ状態で作られるので、環境変数で選ぶストアはローカルの値で判定されます。

これらの結果には、`--json` の出力で `evidence` が付きます。

| `evidence` | 意味 |
|------------|------|
| `manifest` | イントロスペクションの結果を使って判定した。マニフェストにない事実(OAuth の state ストア、キュー、明示的に生成したインメモリストア)はソースの走査で補う |
| `static` | ソースの走査だけで判定した。イントロスペクションが失敗したか省略された場合と、判定がマニフェストの事実を使わない場合(provider の自動検出) |
| `none` | 判定に必要な情報がわからない。`register()` で例外を投げた provider がある場合と、そのセクションを deferred provider が提供するか、自身を説明できないバインディングである場合。キーの末尾は `-unverified` で、結果は必ず警告になる |

イントロスペクションが失敗すると、`check` は理由を書いた advisory の
`introspection-unavailable` を 1 行加え、判定はソースの走査に戻ります。
