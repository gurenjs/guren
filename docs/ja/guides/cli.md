...
`add admin`・`add resource`・`add auth`・`make:feature`・`make:auth` は、いずれも
Inertia ページとそれを描画するコントローラーを生成します。`api` ブループリントで
生成したアプリ（`@guren/inertia-client` 依存も `routes/web.ts`（または
`routes/web.js`）も持たない）では、型検査を通らないコントローラー・描画されない
ページ・どこにもマウントされないルートファイルを残す代わりに、いずれのコマンドも
理由を示して中断し何も生成しません。エンドポイントは `routes/api.ts` に手動で
追加するか、フルスタックアプリをスキャフォールドしてください。

判定には両方の条件が必要です。そのため、依存がワークスペースルートに巻き上げられた
フルスタックのメンバーは `routes/web.ts` の存在が保証となって通常どおり生成され、
`package.json` を読めないアプリも同様に中断されません。単一ファイルを生成する
コマンド（`make:controller`・`make:view`）も Inertia 形の出力を書きますが、
