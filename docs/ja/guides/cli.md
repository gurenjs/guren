...
キャッシュストア、登録中に例外を投げた provider の有無です。いまこの結果を読むのは
これらの判定だけなので、イントロスペクションはデプロイプラグインか Lambda アダプタを
宣言したアプリでだけ行います。セッションや添付ファイルのチェックも今後この結果を読む
ようになります。イントロスペクションは 1 回の実行につき最大 1 回です。マニフェストは
...
| `static` | ソースの走査で判定した。イントロスペクションが失敗したか省略された場合、判定がマニフェストの事実を使わない場合(provider の自動検出)、判定に使う情報をアプリが保証できない場合のいずれか。最後のものは、`register()` で例外を投げた provider がある場合、未設定の環境変数を読む設定がバインドされなかった場合、deferred provider か自身を説明できないバインディングがそのセクションを提供する場合で、メッセージに理由が書かれる |
