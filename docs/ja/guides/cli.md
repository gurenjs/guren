...
- `controller-unparsed:*` は、コントローラのファイルを読み込めたものの構文解析できなかった場合に warn します。そのファイルのアクションは上記のルールから本体が見えません。さらに、そのファイルのクラスを指すルートは、同名のクラスを宣言する別のコントローラファイルの本体で判定され、`controller-name-collision:*` も出ません。
