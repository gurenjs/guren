...
Gurenの国際化は必要なものが一通りそろっています: ファイルベースの翻訳カタログ、リクエスト単位のロケール検出、コントローラーとReactページの翻訳ヘルパー、15言語超の複数形化ルール、型付き翻訳キー、カタログの整合性チェッカーです。
...
ロケールが`ja`に解決された訪問者には、サーバーレンダリングのテキストにもクライアント側のテキストにも日本語カタログが適用され、`<html lang="ja">`も出力されます。追加の配線は要りません。
...
`detect: false`を渡すと`detectLocaleMiddleware`を自分でマウントできます。あるいは`locale`コンテキスト変数を設定する独自ミドルウェア(例: ログインユーザーの保存済み設定から)に置き換えることもできます。下流の`this.t()`、`_i18n`、`<html lang>`はいずれもコンテキスト変数に追従するので、検出後に他のミドルウェアが上書きした場合はそちらが優先されます。
...
クライアント側の翻訳は、補間・複数形・フォールバック解決のいずれもサーバーのデフォルトの挙動と一致します。同じキーはコントローラーで翻訳してもブラウザで翻訳しても同じ結果になります。(カスタム複数形化ルールや`onMissingKey`ハンドラなどサーバー専用のTranslatorカスタマイズは関数のためシリアライズされたpropを越えられず、サーバー側のみに適用されます。)サーバーサイドレンダリングも共有props経由なので追加配線は不要です。
...
`guren check`は翻訳カタログを検証します(CI向けに、`--i18n`を付けるとこの検査だけを実行し、失敗時は非ゼロで終了します):
...
- **プレースホルダの不一致**: 同じキーなのにロケール間で`:name`/`{name}`が異なる場合、翻訳時に変数が失われている可能性が高いです。ロケールによっては意図的にプレースホルダを省くこともあるため、警告として報告するだけでexit codeには影響しません。
- **解決不能なキー**: リテラルのドットを含むプロパティ名・ファイル名。実行時に解決できません。

## サーバーレスとバンドルカタログ

`lang/`はファイルシステムから読まれますが、デプロイ先には`lang/`が含まれません。そのためデプロイビルド(`guren cloudflare:build`、`guren lambda:build`、Vercelのビルド)がビルド時に`lang/<locale>/*.json`を読み、カタログを出力に埋め込みます。既定のディレクトリを使うアプリは、変更なしでデプロイ後も翻訳されます。JSONとして読めないファイルは除外され、ビルドが警告します。翻訳を変えたときは再ビルドと再デプロイが必要です。

ビルドが読むのは`lang/`だけです。`path`を指定したアプリや、Guren のデプロイビルドを使わない環境([サーバーレスガイド](./serverless.md)参照)では、`MemoryLoader`で自分でカタログをバンドルしてください。明示した`loader`はビルドが埋め込んだカタログより優先されます:
...
`loader`オプションは`TranslationLoader`インターフェースを実装するものなら何でも受け付けるため、データベースやリモートサービスからカタログを供給することもできます。なお、codegenと`check --i18n`は規約どおりの`lang/`ディレクトリを読みます。上記のようにJSONファイルを`lang/`に置いたままimportでバンドルすれば、型付きキーとカタログチェックはそのまま使えます。

## テスト

翻訳の挙動は、実アプリを通してテストするのがいちばん簡単です。`TestApp`でラップします([テストガイド](./testing.md)参照):
...
`t()`は既定アプリケーションからマネージャを解決するので、シンプルなスクリプトならこれで足ります。ただしリクエストハンドラでは使わないでください: 共有マネージャの`setLocale()`はプロセス全体の状態なので、並行リクエストが互いのロケールを上書きします。`createApp({ i18n })`で配線したアプリなら、リクエストスコープのヘルパー(`this.t()`、`useTranslation()`)は安全です。リクエストとアプリのコンテナ経由で解決され、共有の可変ロケール状態を通らないためです。
