...
**最初から Bun ネイティブ。** Guren は Bun ランタイム上で動き、HTTP レイヤーには Hono を使います。Node.js の互換シムはありません。Bun の速い起動、TypeScript のネイティブ実行、組み込みテストランナーをそのまま使えます。

**Laravel の開発体験を、TypeScript で。** Laravel の経験があれば、リソースルーティング、`this.inertia()` を備えた `Controller` 基底クラス、`Model.where().orderBy().get()` といったパターンにすぐ馴染めるはずです。経験がなくても心配はいりません。API は「読めば何をするか分かる」設計です。

**エンドツーエンドの型安全。** Drizzle スキーマの型がモデルへ、コントローラーを通って React ページの props へと流れます。カラム名を変更すれば、データベースからブラウザまで、更新が必要な箇所を TypeScript がすべて指摘します。

**コードとつながり続けるプロジェクト知識。** Guren はアーキテクチャ上の意思決定と生成したスペックを、その対象となるエンティティやコードパスに結び付けます。結び付きは検証され、開発中はドキュメント全体をインタラクティブな Docs Graph として見られます。ワークフロー全体は [スペックアンカード開発](./spec-anchored.md) を参照してください。

**バッテリー同梱、ただし強制はしない。** 認証、バリデーション、キャッシュ、キュー、メール、イベント、ブロードキャスト、スケジューリングまで、必要になったときにはすべて揃っています。各サブシステムは ServiceProvider によるオプトイン方式なので、読み込まれるのは使うものだけです。
...
**グローバル状態ではなく registrar 方式のルーター。** 生成されるアプリはルート登録用の registrar を export し、`createApp({ routes })` に渡します。ルーティングはアプリケーションインスタンスごとにスコープされます。

Hono・Next.js・Laravel との踏み込んだ比較と、AI コーディングエージェント向けの設計思想については [Why Guren](./why-guren.md) を参照してください。

## はじめる

コマンドは 4 つだけです。Docker もデータベースサーバーも要りません。新規アプリは最初から SQLite で動きます。
...
2. **[Guren チュートリアル](../tutorials/00-overview.md)**: **初めての方に最もおすすめ。** 空のディレクトリからデプロイまでブログを作る、全 14 章のハンズオンコースです。各章ではひとスライスをコーディングエージェントに委ね、その結果をテストと `guren gate` で判定します。完走すれば、コアコンセプトをひと通り押さえたうえで、次のスライスの委ね方も身についています。
...
チュートリアルの前に軽く見て回りたい方は、[ファーストステップ](./first-steps.md) をどうぞ。1 つのリクエストが各レイヤーをどう通るかを 10 分で辿れます。
