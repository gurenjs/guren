...
先に `bun run dev` を止めてください(ターミナルで Ctrl-C)。前の章で起動した dev サーバーが、3333 番ポートを使ったままです。本番のサーバーは開発サーバーのように次の空きポートへ歩かず、そのまま終了します。
...
もうひとつ、意外なところが変わります。バナーが消え、本番の起動は `[guren] Listening on http://0.0.0.0:3333` の 1 行だけを出力します。リクエストに応答するかどうかは、アプリに尋ねて確かめてください。
...
| `APP_URL` | `.env` の `http://localhost:3333`。`NODE_ENV=production` では `localhost` にしか応答せず、`127.0.0.1:3333` へのリクエストは 403 になる | 公開 URL を設定する。未設定だとホスト認可はオフのままで、警告が出る |
...
- **`db:migrate` が `table sessions already exists` で失敗する。** 第 6 章の `add auth` の比較を、dev サーバーが動いたまま試しています。そのリロードが `add auth` 自身の `sessions` マイグレーションを適用し、あとの `git clean` はフォルダだけを消してテーブルを残しました。`bun run db:status` はそのマイグレーションを orphaned と表示します。`bun run db:reset` は、この章の分も含めて手元のマイグレーションからデータベースを作り直します。その過程で行はすべて消えます。
...
- **`bun run preview` が `Listening on` の 1 行しか出力せず、バナーが出ない。** それが正しい動きです。バナーは開発時のものなので、アプリが応答するかどうかは `/health` に尋ねてください。
