...
この章では Guren アプリを雛形生成し、雛形が何を用意してくれたのかを読みます。それからテストを先に置いて変更をひとつ手で加え、次の変更はコーディングエージェントに委ねて、ハーネスがその仕事をどう検査するかを確認します。最後に、どこでも動かせるコンテナイメージを手にします。以降のすべての章も同じ形で終わります。ゲートが緑で、コミット済みで、出荷できる状態です。
...
- 新規アプリに同梱されているもの: テスト、CI ワークフロー、エージェントハーネス
...
対話的に実行すると、雛形生成ツールは 4 つの質問をします。ここではコマンドラインで先に答えてしまい、このコースが説明するアプリと同じものを作ります。
...
`this.t()` は `lang/en/messages.json` を読み、そのキーも型付きです。`messages.welcome` は存在し、`messages.hello` はコンパイルが通りません。実行時の間違いをコンパイルエラーに変える、Guren の数多くの仕掛けの最初のひとつです。
...
- `guren check` は動いているアプリではなくコードを読みます。すべてのルートが実在するコントローラーメソッドを指しているか、すべての `pages.X` が実在するページファイルを指しているか、各ページの props がコントローラーの送るものと一致しているか。ほかにも、放っておけば実行時に落ちる十数項目を検証します。
- `guren audit` は静的なセキュリティレビューです。バリデーションや認証の無い変更系ルート、生 SQL、ソース内のシークレット、マスアサインメント。新規アプリでは何も言いません。

このコースは、これらのステージが持つひとつの性質に寄りかかっています。コードを書いたのが人間でもエージェントでも、ステージは変わらないという性質です。次の節が成り立つのもそのためです。
...
- **`SessionStart`** は、エージェントの最初のターンの前に `bunx guren context` の出力をコンテキストへ注入します。すべてのモデル、ルート、コントローラー、ページの地図で、末尾にはフレームワークの API シグネチャのダイジェストが付きます。エージェントは `node_modules` を読むことなく、プロジェクトの全体像を把握した状態で始まります。
...
- **`skills/`** はエージェントが求めに応じて従う手順です。`scaffold`(ファイルを手打ちせず `bunx guren make:*` に手を伸ばす)、`feature`、`db-manage`、`guren-api`、`agent-interface`、`ai-agent`、`plugin-authoring`、`dev-workflow`、`github-projects`、`plan-write`、`plan-implement`。
...
以降の各章でこれらをひとつずつ使い、第 8 章では自分で書きます。今は、このあと実際に動くところを見る 2 つの hook に注目してください。
...
実行すると失敗します。これは意図的です。一度も失敗したことのないテストは、何も証明していません。
...
今度は同じ種類の変更をエージェントに任せ、hook の動きを観察します。`guren-blog` の中でエージェントを起動してください(Claude Code なら `claude`)。`SessionStart` hook のおかげで、最初のメッセージには手順 5 で表示したプロジェクトの地図がすでに載っています。こう尋ねます。
...
答えを `.claude/settings.json` と照らして読んでください。3 つの hook すべてと、それぞれが実行するものを挙げているはずです。`guren gate` に触れていなければ、`CLAUDE.md` を読んでいません。仕事を任せる前に、自分のエージェントについて知っておく価値があります。
...
どちらの場合も、受け入れる前に結果をレビューしてください。以降の各章でもエージェントの出力に対して rubric を示します。最初のものは短いです。
...
本番の起動にバナーはありません。サーバーが立ち上がると、コンテナは `[guren] Listening on http://0.0.0.0:3333` の 1 行だけを出力します。これはコンテナ内のワイルドカードのバインドで、手元のマシンからは今までどおり `localhost:3333` です。もう一度 [http://localhost:3333](http://localhost:3333) を開きます。同じページですが、今度はコンテナの中で動くアプリの本番ビルドが返しています。誰のマシンでも同じように動くものです。Ctrl-C で止めてください。注意点が 2 つあり、どちらも第 14 章で解決します。コンテナが開発用の `.env` を読んでいること、そして SQLite ファイルがコンテナの中にあるので、止めると中身がすべて消えることです。
...
**どこでホストするか**は自由に選べます。コースはその選択に依存しません。`bunx guren deploy --target fly` と `--target railway` は、同じ Dockerfile の隣にそれぞれのプラットフォームが求める追加設定を書きます。コンテナイメージが動くホスト(Render、Koyeb、Docker の入った VPS)なら Dockerfile だけで動きます。第 14 章で、Postgres、データベースバックのセッション、その前に立つ CI ゲートを備えた本物のデプロイを一通り行います。

## いまいる場所

- SSR と SQLite で動く Guren アプリ。git 管理下で、自分のコミットが 3 つ。
- 赤と緑の両方を見たテストスイート。
- CI が実行するゲート。そしてそれが自分で実行するものと同じだという知識。
- `guren check` と `guren gate` の指摘を、自分が見るより先にエージェントへ返すハーネス。
- Dockerfile。
...
- **手順 6 で何も変えていないのに `bun test` が通る。** 赤のステップを走らせる前に `Home.tsx` を置き換えています。順序が大事です。テストが先、失敗を確認してから変更です。
...
以下はどれも後の章が必要とするものではありません。ファイルを変える演習はブランチ (`git switch -c exercise/…`) で行い、終わったらブランチだけでなく作業そのものを戻してください。`git switch main` のあと `git reset --hard` と `git clean -fd`、最後に `git branch -D exercise/…` です。何が消えるかは先に `git clean -fdn` で見られます。一度もコミットしていないブランチは何も抱えていないので、消しても何も消えません。編集も新しいファイルも作業ツリーに残ったままです。`restore` ではなく `reset --hard` なのは、途中で `git add` したものまで戻すためです。とくに `db/migrations/` の下に残ったものは、次の `bun run dev` が黙ってデータベースに適用します。

1. 雛形が書いた workflow は `bunx guren gate --deps` を走らせます。ここまで走らせてきたのは `bunx guren gate` です。長いほうを走らせてください。`--deps` は何を足していますか。ネットワークの無いマシンではどうなりますか。
2. `bunx guren doctor --next` を走らせてください。報告されたもののうちひとつを選び、それを直すと何が得られるかを説明してください。まだ作っていない本番アプリ向けの提案も混ざっています。どれがそうかも答えてください。

## 次へ

[第 2 章: リクエストをひとつ、手で](./02-one-request-by-hand.md) では、空のファイルからルート、コントローラー、ページをテストを先に置いて組み上げ、2 つ目のページをエージェントに委ねます。
