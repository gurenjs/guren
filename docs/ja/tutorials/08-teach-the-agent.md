...
1. このアプリにあって、どのチェックも強制していない約束事について rule をもう 1 つ書いてください。候補のひとつは「すべてのページコンポーネントは `Props` インターフェースを宣言する」です。第 13 章の `spec:generate` がそれを読むからです。20 行以内に収め、なぜどのコマンドにも見えないのかを rule 本文に書いてください。
