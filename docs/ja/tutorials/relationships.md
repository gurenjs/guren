...
## 7. 作ったものを Docs Graph で結び付ける

動いているアプリは、コメントのフローが **何をするか** を証明しています。次は構造を要約するビューを生成し、コメントの著者をこの方法で扱う **理由** を記録します。

まず、コードから ER、ドメイン、画面、モジュールのビューを導出します。

```bash
bunx guren spec:generate
```

生成された Markdown は `docs/spec/` 配下に保存されます。特に `er.md` には 3 つのテーブルと外部キーが、`domain.md` にはここで宣言したモデルのリレーションシップが表示されます。
...
ターミナルから `Comment` の近傍を照会します。

```bash
bunx guren docs:graph --entity Comment
```

最後に `bun run dev` を動かしたまま [http://localhost:3333/_guren/docs](http://localhost:3333/_guren/docs) を開きます。新しい ADR と `Comment` エンティティを見つけ、エッジを辿って関連コントローラーを確認し、生成された ER ビューとドメインビューも開いてください。CLI と同じ検証済みの関係を、今度は視覚的な画面から読んでいます。

ビューアーはローカル限定、読み取り専用、開発時限定です。文書形式、trust metadata、ドリフト検証、エージェント向けワークフローの詳細は [スペックアンカード開発](../guides/spec-anchored.md) を参照してください。
