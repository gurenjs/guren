...
1. ブランチを切って `db/schema.ts` に列を足し、`bunx guren gate` を走らせてください。4 つのビューのうちドリフトしたのはどれで、しなかったのはどれですか。しなかった理由を、各ビューが何を読んでいるかから説明してください。終わったらブランチを削除してください。
2. このアプリには、まだ文書化されていない決定があります。タグの正規化をモデルではなくバリデーターで行っていることです。ADR を書き、`Post` にリンクし、`bunx guren check --docs` を通してください。そのうえで、コードだけでは分からず、そのファイルから得られるものは何かを答えてください。
