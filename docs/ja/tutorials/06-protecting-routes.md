...
列を必須にする作業は、次のスライスとしてエージェントに任せます。
...
[第 7 章: 認可、そしてゲートに見えないもの](./07-authorization.md) では、ポリシーを使って投稿の編集と削除を著者だけに許可します。そのあと認可には触れずにエージェントへ機能を頼み、用意してきた安全装置のうちどれがそれに気づくかを確かめます。
