...
ここまでの 11 章では、人が読むためのブログを作ってきました。この章では、同じアプリを別の種類の呼び出し元に開きます。すでに書いたルートを通して投稿を読み、下書きを公開するエージェントです。

2 つ目の API を作ることはありません。ツールとは、すでに作ったルートに名前と、エージェントが読める契約と、呼ぶと何が起きるかの主張を加えたものです。注目してほしいのは、その主張をした途端にルートの扱いが変わる点です。チェックが厳しくなり、第 7 章では警告するしかなかった穴で、ついにそのひとつが失敗します。

**この章で学ぶこと:**

- ツールを構成する要素: 名前、入力スキーマ、出力の形、注釈
- ページのルートとツール前提のルートで出力の記述方法が違う理由と、入れ替えたときに壊れるもの
- 認可の無い、変更を伴うツールをビルドの失敗に変えるルール
- 自分のツールを、エージェントとまったく同じやり方でテストから呼ぶ方法
- Guren アプリにある 2 つの MCP エンドポイントと、その使い分け
...
`posts.show` はツールに必要なものを、宣言以外すべて持っています。その宣言を足します。
...
ツールのテストに、モデルのクライアントもネットワークも要りません。`TestApp` はサーバーと同じ導出を公開し、呼び出しはほかのテストリクエストと同じ `fetch` を通って出ていきます。
...
最後のテストは二度読む価値があります。第 7 章で書いたのと同じ 403 に、HTTP リクエストではなくツール呼び出しから到達していて、通る理由も同じです。ルートが走るからポリシーが走ります。ツール呼び出しは裏口ではなく、本物のミドルウェアチェーンを通る本物のリクエストで、書いたガードはひとつ残らずその手前に立っています。
...
宣言するルートは 2 つで、必要なものはそれぞれ違います。

`posts.index` もページのルートなので、`posts.show` と同じ扱いになります。エージェントがページングを知れるように `query` の契約を、形を宣伝できるように `resource` のヒントを付けます。

`posts.publish` は違います。その違いこそがここでの教材です。ブラウザにはリダイレクトで応答します。フォームには正しい応答ですが、エージェントには役に立ちません。リダイレクトは投稿を運ばないからです。そこでアクションに 2 通りの応答を覚えさせ、ルートはエージェントに JSON 版を約束します。
...
アクションはひとつ、相手は 2 種類、ポリシーはひとつ。認可も更新もイベントも分岐より前にあり、違うのは最後の 1 行だけです。既存のアクションをツールにするときは、いつでもこの形を使ってください。この章が 2 つ目のコントローラーを作らないのもそのためです。
...
`posts.publish` には契約が 3 つあり、どれも欠かせません。`params` はエージェントが送る id に型を付けます。空のオブジェクトである `body` は、このアクションがペイロードを取らないことをツールが言うための書き方です。これが無いと、エージェントには何を送ればよいか分からない、とチェックが警告します。`output` は約束で、実際に走るのはこれです。合致しない 2xx のレスポンスは呼び出し元に届かず 500 になり、スキーマが名前を挙げていないフィールドは削ぎ落とされます。ページのルートは代わりに `resource` を使います。こちらは型レベルだけのものです。ブラウザに Inertia のページを返すルートに `output` スキーマを置くと、そのページを検証しようとしてしまうからです。
...
ここまでをコミットしてください。次の手順で、それを意図的に壊します。
...
何が変わって何が変わっていないかを読んでください。`guren audit` はこのルートを相変わらず通します。`auth` グループの中にあるのでユーザーは要求されていて、audit が問うのはそこまでだからです。ルートのテストなら捕まえますし、そのテストはすでに書いてあります。チェックが加えるのは、テストを 1 件も走らせずに*ビルド*が拒むようになる点です。変更を伴うツールは、別の種類の約束だからです。一度も顔を合わせることのない呼び出し元に差し出したアクションであり、「認証済みの何者か」は人ではありません。
...
また緑です。この章のハーネス上のてこは、この `--ci` です。`guren gate` が実行し、エージェントが終わったと言ったときには `Stop` hook が実行します。ルートをツールとして公開しておきながらポリシーを忘れたエージェントは、その仕事を終わったことにできません。


## 5. コメントのツールを仕様化する

投稿を読めるエージェントは、それに答えられてしかるべきです。人と同じルールのもとで。
...
真ん中のテストは覚えておく価値があります。よくない引数を送ったツール呼び出しは、プロトコルの障害ではなく、バリデーター自身のメッセージを運ぶエラー結果として返ってきます。エージェントは人と同じことを、同じ言葉で、同じスキーマから告げられます。
...
**手元にエージェントが無い場合は、** バリデーターに契約を 2 つ足します。
...
ツールは 5 つ。読むものが 2 つ、何かを変えるものが 3 つ。そのどれにも、フレームワークから見えるアビリティか、アクションが強制するポリシーが付いています。
...
アプリはもうツールを持っています。それを本物のエージェントに渡すには、プラグインを 1 本入れるだけです。
...
これが `/mcp` をマウントします。本番で動く本物のエンドポイントで、`tools/list` には宣言したツールで答え、呼び出しはどれも自分のミドルウェアを通る通常のリクエストとして実行されます。bearer トークン、トークンごとのツールのスコープ、レート制限で守られていて、トークンストアが、そのためにテーブルが要ります。それは第 14 章の仕事で、本番に出るためのほかの作業と一緒に扱います。

エディターがすでに話しかけているエンドポイントと混同しないでください。`GUREN_MCP=1` は開発時にだけ `/_guren/mcp` をマウントし、ループバックインターフェース上にいない呼び出し元をすべて拒みます。そのツールは `guren_check`、`guren_gate`、`guren_get_context` とその仲間で、働きかける先は*プロジェクト*、つまりアプリを書くエージェントのためのものです。プラグインのツールは*アプリケーション*のもので、アプリを使うエージェントのためのものです。同じプロトコルで、向きは逆になります。前者は、第 8 章のハーネスがすでに設定してくれていました。

プラグインを入れる前の `/mcp` にルートはありませんが、POST すると 403 の `CSRF token mismatch` が返ります。404 にはなりません。CSRF の検証はルーティングより前に走るので、トークンを持たないリクエストは存在しないパスでも同じ答えになります。

## いまいる場所

- ツールでもあるルートが 5 つ。そのどれにも、エージェントが読む入力スキーマと、頼りにできる出力の形がある。
- ブラウザとエージェントに、最後の 1 行だけ違う応答を返し、それ以外はまったく同じことをするアクションがひとつ。
...
[第 13 章: 古びないドキュメント](./13-documented.md) では、アプリに自分自身を記述させます。生成される ER 図とドメインのビュー、エージェントがエンティティに触れる前に読むドキュメント、そしてそのどちらかがコードからずれたときに失敗するゲートです。
