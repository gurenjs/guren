...
質問には第 3 章と同じ手順で答えます。ページで **refuse** を選んでフィードバックをコピーし、次のプロンプトに添えて Claude Code のセッションに送ります。
...
[第 7 章: アプリが動いたとき](./07-when-the-application-moves.md) では、同僚が並行してコードを変更する中で、この計画を実装します。
