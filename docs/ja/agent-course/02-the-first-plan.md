...
- `docs/plans/meetups/plan.json` があります。まだ誰も承認していない下書きです。
- レビューページを描画しました。このページは git の対象外です。
- 直すものの一覧ができました。未決の質問が 1 つと、足りない振る舞いが 5 つです。
...
1. レビューページの **Entity** メニューを **All** のままにして、**Changes only** をオンにしてください。何が表示されなくなりましたか。また、ほとんどが追加で占められる計画で、この表示が役立つのはなぜでしょうか。
2. **Tasks & acceptance** から振る舞いを 1 つ選び、それがどんなテストになるかを 1 文で書いてください。実際のテストは第 4 章で出てきます。

<details>
<summary>演習 1: ヒントと答えの例</summary>

**Entity** メニューは、各要素のエンティティを、その要素を含むタスクから決めます。どのタスクにも含まれないモデルは、モデル自身の名前のエンティティに入ります。**Changes only** は、change が `existing` の要素を隠します。

この計画で `existing` の要素は `model.user` とその `id` カラムの 2 つで、消えるのはちょうどこの 2 つです。「… of … elements shown」の件数も 2 つ減ります(どちらもエンティティ `User` に属するので、**Entity** メニューで **Meetup** を選んでも隠れます)。残るのは計画が新しく作る要素だけで、レビューで読むべきものはそれです。`existing` の要素は、設計がつながる先を示すだけです。質問はタブの上に表示され、絞り込みの対象になりません。
...
振る舞いには、ルート、操作する人 (actor)、`given` の前提、`expect` の期待する結果がすでに書かれています。テストは同じ内容をコードで表したものです。

`AC-meetups-4` なら「勉強会の主催者ではないユーザーでサインインし、正しい本文で `PUT /meetups/:id` を送ると 403 が返る」となります。第 4 章のテストも同じ形で、Grace が主催する勉強会に Ada が送ると 403 が返ることを確かめています。
